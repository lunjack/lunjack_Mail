 ...
 */
const _isFamilySupported = (family, allowInternal) => {
 ...
 */
const _resolver = (family, hostname, options, callback) => {
    options = options || {};
    // 检查地址族是否支持
 ...
 */
const _formatDNSValue = (value, extra) => {
...
const parseConnectionUrl = str => {
    ...
    [urllib.parse(str, true)].forEach(url => {
        ...
        if (url.auth) {
            ...
            options.auth.user = auth.shift(); // 用户名
            options.auth.pass = auth.join(':'); // 密码
        }
 ...
 */
const resolveContent = (data, key, callback) => {
    let promise;
