...
const { callbackPromise, formatRecipients, initSmtpConstructor, createProxyConnection } = require('./shared');
/**
            ...
            connection.connect(() => {
                ...
                if (auth && (connection.allowsAuth || options.forceAuth)) {
                    connection.login(auth, err => {
                        auth && auth !== this.auth && auth.oauth2?.removeAllListeners();
                        if (returned) return;
