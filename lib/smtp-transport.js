...
class SmtpTransport extends EventEmitter {
    ...
    send(mail, callback) {
        getSocket(this.options, (err, socketOp) => {
            ...
            const { options: newOp, connection } = createSmtpConnection(this, socketOp, SmtpConnection),
                ...
                // 发送消息
                sendMessage = () => {
                    const { envelope, readStream, from, to, messageId } = prepareMessageForSending(mail, this.logger);//邮件预处理
    ...
    // 释放资源
    close() {
        this.auth?.oauth2?.removeAllListeners(), this.emit('close');
    }
}
