...
class StreamTransport {
    constructor(options = {}) {
        ...
        this.name = 'StreamTransport', this.version = PK.version;
    ...
    send(mail, done) {
        ...
        message.keepBcc = true;                           // 在输出中保留 Bcc 字段
        ...
        setImmediate(() => {
            ...
            try {
                stream = message.createReadStream();  // 创建消息读取流
            } catch (E) {
            ...
            // 处理可读流(监听可读事件,错误及结束事件)
            stream.on('readable', () => handleReadableStream(stream, chunks, chunklen))
                .once('error', err => handleError(err, '为 %s 创建消息失败。%s'))
                .on('end', () => done(null, { envelope, messageId, message: Buffer.concat(chunks, chunklen.value) }));
        });
    }
}
