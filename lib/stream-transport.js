        ...
        setImmediate(() => {
            ...
            if (!this.options.buffer) {
                readStream.once('readable', () => {
                    done(null, { envelope, messageId, message: readStream }); // 流可读时才认为成功
                }).once('error', err => handleError(err, '为 %s 创建消息流失败。%s'))
            }

            // 将流数据读取到Buffer中
            resolveStream(readStream, (err, value) => {
                err ? handleError(err) : done(null, { envelope, messageId, message: value });
            });
        });
    }
}
