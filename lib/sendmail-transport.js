...
class SendmailTransport {
    constructor(options = {}) {
        ...
        this.name = 'SendmailTransport', this.version = PK.version;
        ...
        this.logger = getLogger(this.options, { component: component || 'sendmail' });
     ...
     */
    send(mail, done) {
        const { message, data } = mail;
        message.keepBcc = true;                                       // 保留Bcc字段
        const envelope = data.envelope || message.getEnvelope();      // 获取信封信息
        const messageId = message.messageId();                        // 获取消息ID
        ...
        // 增强错误处理
        const handleError = (err, tnx, context) => {
            this.logger.error({ err, tnx, messageId }, context, messageId, err.message);
            cleanUp(err);
        };
        ...
        const sourceStream = message.createReadStream(); // 获取源数据流
        sourceStream.once('error', err => handleError(err, 'source', '创建邮件数据流失败;%s'));
