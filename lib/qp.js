...
function qpWrap(str, lineLength) {
    ...
    while (pos < len) {
        ...
        else if ((match = line.slice(-lineMargin).match(/\n.*?$/))) {
            // 截断到最近的换行符
        ...
        else if (line.length > lineLength - lineMargin && (match = line.slice(-lineMargin).match(/[ \t.,!?][^ \t.,!?]*$/)))
            line = line.slice(0, line.length - (match[0].length - 1));  // 截断到最近的空格
        else if (line.match(/[=][\da-f]{0,2}$/i)) {
            // 将不完整的编码序列推到下一行
            if ((match = line.match(/[=][\da-f]{0,1}$/i))) line = line.slice(0, line.length - match[0].length);

            // 确保UTF-8序列不被分割
            while (line.length > 3 && line.length < len - pos && !line.match(/^(?:=[\da-f]{2}){1,4}$/i)
        ...
        if (pos + line.length < len && line.slice(-1) !== '\n') {
            if (line.length === lineLength && line.match(/[=][\da-f]{2}$/i)) line = line.slice(0, line.length - 3);
