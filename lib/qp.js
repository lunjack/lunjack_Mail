...
function qpWrap(str = '', lineLength = 76) {
    ...
    while (pos < len) {
        ...
        if (line.endsWith('\n')) {
            ...
            line = line.slice(0, line.length - (match[0].length - 1)); // 截断到最近的换行符
            addLineAndUpdatePos(line);
            continue;
        }
...
class QpEncoder extends Transform {
    constructor(options = {}) {
        ...
        if (lineLength !== false) this.options.lineLength = lineLength || 76; // 设置行长度，默认为76

        this._curLine = '';    // 当前行缓存
        this.inputBytes = 0, this.outputBytes = 0;                            // 统计输入输出字节数
    }
