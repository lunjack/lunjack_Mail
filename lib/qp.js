...
const { regex, resetRegex, } = require('./regexs');
...
function qpWrap(str, lineLength) {
    ...
    while (pos < len) {
        ...
        // 处理已有的CRLF换行
        if ((match = line.match(regex.CRLF))) {
        ...
        else if ((match = line.slice(-lineMargin).match(regex.TRAILING_NEWLINE))) {
            // 截断到最近的换行符
        ...
        else if (line.length > lineLength - lineMargin && (match = line.slice(-lineMargin).match(regex.TRAILING_SPACE_OR_PUNCTUATION)))
            line = line.slice(0, line.length - (match[0].length - 1));  // 截断到最近的空格
        else if (line.match(regex.INCOMPLETE_ENCODING)) {
            // 将不完整的编码序列推到下一行
            if ((match = line.match(regex.INCOMPLETE_ENCODING_SHORT))) line = line.slice(0, line.length - match[0].length);

            resetRegex(regex.COMPLETE_ENCODING);
            // 确保UTF-8序列不被分割
            while (line.length > 3 && line.length < len - pos && !line.match(regex.UTF8_ENCODING)
                && (match = line.match(regex.COMPLETE_ENCODING))) {
        ...
        if (pos + line.length < len && line.slice(-1) !== '\n') {
            resetRegex(regex.COMPLETE_ENCODING);
            if (line.length === lineLength && line.match(regex.COMPLETE_ENCODING)) line = line.slice(0, line.length - 3);
...
class QpEncoder extends Transform {
    ...
    _transform(chunk, encoding, done) {
        ...
        if (this.options.lineLength) {
            ...
            qp = qp.replace(regex.LAST_LINE, (match, lineBreak, lastLine) => {
