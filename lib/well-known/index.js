...
Object.keys(SERVICES).forEach(key => {
    ...
    addServiceEntry(key);                              // 处理主键
    (service.aliases ?? []).forEach(addServiceEntry);  // 处理别名
    (service.domains ?? []).forEach(addServiceEntry);  // 处理域名
});
...
function normalizeKey(key) {
    resetRegex(regexs.NON_ALPHANUMERIC_DOT_HYPHEN);
    return key.replace(regexs.NON_ALPHANUMERIC_DOT_HYPHEN, '').toLowerCase();
}
