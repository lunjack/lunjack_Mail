...
function _relaxedHeaderLine(line) {
    const rGCOL = resetRegex(regexs.GLOBAL_CRLF_OR_LF);
    const rGW = resetRegex(regexs.GLOBAL_WHITESPACE);
...
function generateDKIMSignature(headers, hashAlgo, bodyHash, options = {}) {
    ...
    // 生成DKIM签名头（不包含签名值部分）
    const dkimHeader = _generateDKIMHeader(domainName, keySelector, result.fieldNames, hashAlgo, bodyHash);
