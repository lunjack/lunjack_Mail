...
function _relaxedHeaders(headers, fieldNames, skipFields) {
    ...
    (skipFields || '')
        .toLowerCase()
        .split(':')
        .forEach(field => { skip.add(field.trim()); });         // 去除空白并添加到跳过集合
    ...
    (fieldNames || '')
        ...
        .forEach(field => { includedFields.add(field.trim()); }); // 添加到包含字段集合
...
function _relaxedHeaderLine(line) {
    resetRegex(regexs.GLOBAL_CRLF_OR_LF);
    resetRegex(regexs.GLOBAL_MULTIPLE_WHITESPACE);
    return line
        .substring(line.indexOf(':') + 1)                // 去除字段名部分
        .replace(regexs.GLOBAL_CRLF_OR_LF, '')           // 移除换行符
        .replace(regexs.GLOBAL_MULTIPLE_WHITESPACE, ' ') // 将连续空白压缩为单个空格
        .trim();                                         // 去除首尾空格
}
...
function generateDKIMSignature(headers, hashAlgo, bodyHash, options = {}) {
    ...
    const result = _relaxedHeaders(headers, fieldNames, options.skipFields);

    // 生成DKIM签名头（不包含签名值部分）
    let dkimHeader = _generateDKIMHeader(options.domainName, options.keySelector, result.fieldNames, hashAlgo, bodyHash);
    ...
    resetRegex(regexs.SIGNATURE_FOLDING);
    return dkimHeader + signature.replace(regexs.SIGNATURE_FOLDING, '$&\r\n ').trim();
}
