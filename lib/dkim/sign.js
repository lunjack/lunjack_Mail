...
function _relaxedHeaders(headers, fieldNames, skipFields) {
    const includedFields = new Set(); // 存储最终需要包含的字段
    const skip = new Set();           // 存储需要跳过的字段
    const headerFields = new Map();   // 存储字段名和对应的规范化值
...
function _relaxedHeaderLine(line) {
    const CRLF_ = resetRegex(regexs.GLOBAL_CRLF_OR_LF);
    const WHITESPACE = resetRegex(regexs.GLOBAL_WHITESPACE);
    // 去除字段名部分,移除换行符,将连续空白压缩为单个空格,去除首尾空格
    return line.substring(line.indexOf(':') + 1).replace(CRLF_, '').replace(WHITESPACE, ' ').trim();
}
...
function generateDKIMSignature(headers, hashAlgo, bodyHash, options = {}) {
    ...
    const { headerFieldNames, skipFields, domainName, keySelector, privateKey } = options;
    const fieldNames = headerFieldNames || defaultFieldNames;        // 使用选项中的头字段列表或默认列表
    const result = _relaxedHeaders(headers, fieldNames, skipFields); // 获取经过"relaxed"规范化的头数据及字段列表

    // 生成DKIM签名头（不包含签名值部分）
    let dkimHeader = _generateDKIMHeader(domainName, keySelector, result.fieldNames, hashAlgo, bodyHash);

    let signer, signature;
    // 将DKIM签名头自身添加到规范化数据中（用于生成签名）
    result.headers += 'dkim-signature:' + _relaxedHeaderLine(dkimHeader);

    signer = createSign(('rsa-' + hashAlgo).toUpperCase()); // 创建签名对象，使用指定的哈希算法
    signer.update(result.headers);                          // 更新要签名的数据（规范化后的头数据）
    try {
    ...
    const SF = resetRegex(regexs.SIGNATURE_FOLDING);
    return dkimHeader + signature.replace(SF, '$&\r\n ').trim();
}
