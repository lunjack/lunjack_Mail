...
class MessageParser extends Transform {
    ...
    _transform(chunk, encoding, callback) {
        ...
        try {
            let headersFound = this.checkHeaders(chunk); // 检查并处理头信息
    ...
    checkHeaders(data) {
        ...
        for (let i = 0, len = lblen + data.length; i < len; i++) {
            let chr;
            if (i < lblen) chr = this.lastBytes[i]; // 如果在lastBytes中, 使用lastBytes中的字节
            else chr = data[i - lblen];             // 否则使用data中的字节
            // 检查是否有两个连续的换行符
            if (chr === 0x0a && i) {
                // 定义pr1为当前字节的前一个字节的值
        ...
        if (this.headersParsed) {
            this.headerChunks.push(data.subarray(0, headerPos));
            this.rawHeaders = Buffer.concat(this.headerChunks, this.headerBytes);
            this.headerChunks = null;
            this.emit('headers', this.parseHeaders());
            // 如果数据块中还有剩余的消息体，则推送剩余的消息体
            if (data.length - 1 > headerPos) {
                let chunk = data.subarray(headerPos);
                this.bodySize += chunk.length;
                setImmediate(() => this.push(chunk)); // 异步推送剩余的消息体
            }
            return true;
        }
        // 否则将整个数据块添加到头信息块中
    ...
    // 解析原始头信息为键值对数组
    parseHeaders() {
        let lines = (this.rawHeaders || '').toString().split(regexs.HEADER_LINE_BREAK); // 按行拆分
        // 处理折叠的头信息行
        for (let i = lines.length - 1; i > 0; i--) {
            // 如果当前行是空白行，则将其与前一行合并
            if (regexs.FOLDED_HEADER_LINE.test(lines[i])) {
                lines[i - 1] += '\n' + lines[i];
                lines.splice(i, 1);
            }
        }
        return lines
            ...
            .map(line => ({
                key: line.slice(0, line.indexOf(':')).trim().toLowerCase(),
                line
            }));
    }
}
