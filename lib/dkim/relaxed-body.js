...
class RelaxedBody extends Transform {
    ...
    updateHash(chunk) {
        ...
        let lines = fullStr.split(regex.LINE_SPLIT);

        // 如果最后一部分不是完整的行结束符，保留作为余数
        if (!regex.TRAILING_LINE_BREAK.test(fullStr)) this.remainder = lines.pop() || '';
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            // 处理行内容
            let line = lines[i];
            line = line.replace(regex.TRAILING_WSP, '');  // 移除行尾空白字符（WSP）
            resetRegex(regex.MULTIPLE_WSP);
            line = line.replace(regex.MULTIPLE_WSP, ' '); // 将行内多个空白字符替换为单个空格
        ...
        if (this.remainder) {
            ...
            resetRegex(regex.MULTIPLE_WSP);
            let processedRemainder = this.remainder.replace(regex.MULTIPLE_WSP, ' ');
    ...
    _flush(callback) {
        try {
            ...
            if (this.remainder) {
                // 对余数应用规范化规则
                resetRegex(regex.MULTIPLE_WSP);
                let processedRemainder = this.remainder.replace(regex.TRAILING_WSP, '').replace(regex.MULTIPLE_WSP, ' ');
