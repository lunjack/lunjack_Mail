...
class RelaxedBody extends Transform {
    ...
    constructor(options = {}) {
        super(options);
        this.bodyHash = createHash(options.hashAlgo || 'sha1'); // 初始化哈希计算器
        this.remainder = '';                                    // 存储未处理完的数据片段（跨块的未完成行）
        this.byteLength = 0;                                    // 记录已处理的字节长度
    ...
    updateHash(chunk) {
        ...
        let fullStr = this.remainder + chunk.toString('binary');

        this.remainder = '';  // 清空余数，将在处理过程中重新收集

        if (fullStr.length === 0) return; // 如果合并后字符串为空，无需处理

        let lines = fullStr.split(regexs.LINE_SPLIT); // 使用正则表达式按行分割，保留行结束符信息
        // 如果最后一部分不是完整的行结束符，保留作为余数
        if (!regexs.TRAILING_LINE_BREAK.test(fullStr)) this.remainder = lines.pop() || '';
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            resetRegex(regexs.MULTIPLE_WSP);
            line = line.replace(regexs.MULTIPLE_WSP, ' '); // 将行内多个空白字符替换为单个空格
        ...
        if (this.remainder) {
            ...
            resetRegex(regexs.MULTIPLE_WSP);
            let processedRemainder = this.remainder.replace(regexs.MULTIPLE_WSP, ' ');
            processedStr += processedRemainder;
        }

        let processedChunk = Buffer.from(processedStr, 'binary'); // 将处理后的字符串转换回Buffer
    ...
    _flush(callback) {
        try {
            ...
            if (this.remainder) {
                // 对余数应用规范化规则
                resetRegex(regexs.MULTIPLE_WSP);
                let processedRemainder = this.remainder.replace(regexs.TRAILING_WSP, '').replace(regexs.MULTIPLE_WSP, ' ');
                let remainderChunk = Buffer.from(processedRemainder, 'binary');

                if (this.debug) this._debugBody.push(remainderChunk); // 调试模式：记录处理后的余数

                this.bodyHash.update(remainderChunk);     // 更新哈希值
                this.push(remainderChunk);                // 将处理后的余数推送到输出流
                this.byteLength += remainderChunk.length; // 更新已处理的字节长
            }
