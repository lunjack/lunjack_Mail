...
class RelaxedBody extends Transform {
    ...
    constructor(options = {}) {
        ...
        this.bodyHash = createHash(hashAlgo || 'sha1'); // 初始化哈希计算器
        this.remainder = '';                            // 存储未处理完的数据片段（跨块的未完成行）
        this.byteLength = 0;                            // 记录已处理的字节长度

        // 调试模式相关设置
        this.debug = debug;
        this._debugBody = debug ? [] : false;
    }
    ...
    updateHash(chunk) {
        ...
        const lines = fullStr.split(regexs.LINE_SPLIT); // 使用正则表达式按行分割，保留行结束符信息
        if (!regexs.TRAILING_LINE_BREAK.test(fullStr)) this.remainder = lines.pop() || '';  // 如果最后不是完整的行结束符，保留作为余数
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            line = line.replace(M_WPS, ' ');               // 将行内多个空白字符替换为单个空格
        ...
        if (this.remainder) {
            ...
            const M_WPS = resetRegex(regexs.MULTIPLE_WSP);
            const pRemainder = this.remainder.replace(M_WPS, ' ');
    ...
    _flush(callback) {
        try {
            ...
            if (this.remainder) {
                // 对余数应用规范化规则
                const M_WPS = resetRegex(regexs.MULTIPLE_WSP);
                const pRemainder = this.remainder.replace(regexs.TRAILING_WSP, '').replace(M_WPS, ' ');
                const remainderChunk = Buffer.from(pRemainder, 'binary');
