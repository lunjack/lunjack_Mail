...
class DKIMSigner {
    ...
    constructor(options = {}, keys, input, output) {
        ...
        this.hashAlgo = hashAlgo || DKIM_ALGO;

        this.cacheDir = cacheDir || false;
        this.keys = keys;
        this.chunks = [];
        this.chunklen = 0;
        this.readPos = 0;
    ...
    createReadCache() {
        ...
        this.cache.once('close', () => this.cleanup());
    ...
    sendSignedOutput() {
        ...
        const signNextKey = () => {
            ...
            const { domainName, keySelector, privateKey } = this.keys[keyPos++];
            const { headerFieldNames, skipFields } = this.options;
            // 定义 DKIM 签名字段
            const dkimField = signer(this.headers, this.hashAlgo, this.bodyHash,
    ...
    createWriteCache() {
        ...
        this.cache = fs.createWriteStream(this.cachePath);  // 将剩余数据管道传输到缓存文件
        // 处理缓存流的错误和关闭事件
        this.cache.once('error', err => {
            this.cleanup();                      // 清理缓存文件
        ...
        this.cache.once('close', () => this.sendSignedOutput());
...
class DKIM {
    ...
    sign(input, extraOptions) {
        ...
        // 创建 DKIM 签名器实例，使用最终的合并选项
        const signer = new DKIMSigner(newOptions, this.keys, inputStream, output);

        // 异步启动签名流处理
        setImmediate(() => {
            signer.signStream();
            // 如果有写入值，则在下一个事件循环中结束输入流
            if (writeValue) setImmediate(() => inputStream.end(writeValue));
        });
