...
class MailComposer {
    ...
    getAttachments(findRelated = false) {
        ...
        // 处理日历事件附件
        const { icalEvent: mIcalEvent } = this.mail;
        if (mIcalEvent) {
            eventObject = typeof mIcalEvent === 'object' && (mIcalEvent.content || mIcalEvent.path || mIcalEvent.href || mIcalEvent.raw)
                ? { ...mIcalEvent } : { content: mIcalEvent };

            Object.assign(eventObject, {
                contentType: 'application/ics',
                filename: eventObject.filename || 'invite.ics',
                headers: { ...eventObject.headers, 'Content-Disposition': 'attachment', 'Content-Transfer-Encoding': 'base64' }
            });
        }
        ...
        const processedAttachments = attachments.map((attachment, i) => {
            ...
            const data = {
                ...
                ...(filename && { filename }), ...(cid && { cid }), ...(raw && { raw }),
            ...
            // 标准化路径并设置内容
            this._normalizeAttachmentPaths(processedAttachment);

            if (path) data.content = { path };
            else if (href) data.content = { href, httpHeaders: processedAttachment.httpHeaders };
            else if (!raw) data.content = processedAttachment.content || '';
    ...
    getAlternatives() {
        ...
        [].concat(text || []).concat(watchHtml || []).concat(amp || []).concat(html || []).concat(eventObject
            ...
            .forEach(alternative => {
                // 处理dataURL格式的内容
                if (regexs.DATA_URL.test(alternative.path || alternative.href)) alternative = this._processDataUrl(alternative);
