...
class MailComposer {
    ...
    getAttachments(findRelated = false) {
        ...
        const pAttachments = attachments.map((attachment, i) => {
            ...
            const { filename, path, href, cid, raw, encoding, headers, contentType: cType, content = '', httpHeaders } = pAtt;
            ...
            const contentDisposition = pAtt.contentDisposition || (isMessageNode || (isImage && cid) ? 'inline' : 'attachment');
            const contentTransferEncoding = 'contentTransferEncoding' in pAtt ? pAtt.contentTransferEncoding
    ...
    getAlternatives() {
        ...
        // 处理替代内容对象
        const processAlt = (letValue, v, defaultContentType) => {
        ...
        // 处理日历事件
        if (icalEvent) {
            const icalEvent_ = processAlt(icalEvent_, icalEvent, null);

            eventObject = {
                ...icalEvent_, filename: false,
                contentType: `text/calendar; charset=utf-8; method=${(icalEvent_.method || 'PUBLISH').toString().trim().toUpperCase()}`,
                headers: icalEvent_.headers || {},
                ...(icalEvent_.content && typeof icalEvent_.content === 'object' && { content: { ...icalEvent_.content, _resolve: true } })
            };
        }
        ...
        // 将所有替代内容合并到数组中
        const alternatives_ = Array.prototype.concat.call([], text_, html_, watchHtml_, amp_, eventObject, alternatives)
            .filter(Boolean).map(originalAlternative => {
                const oAtive = this._prepareAttachmentObject(originalAlternative); // 预处理附件
                const { contentType, filename, path, href, raw, content = '', contentTransferEncoding, encoding, headers } = oAtive;
                const data = ({
    ...
    _createContentNode(parentNode, element = {}) {
        ...
        const eEncoding = encoding.toString().toLowerCase().replace(rEF, '');
    ...
    _processDataUrl(element) {
        const { path, href, contentType = 'application/octet-stream' } = element;
        const dataUrl = path || href;
        ...
        element.contentType = parsedDataUri.contentType || contentType;
