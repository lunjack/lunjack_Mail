...
class MailComposer {
    ...
    getAttachments(findRelated = false) {
        ...
        let iEvent, eventObject;

        // 处理日历事件附件
        if (icalEvent) {
            iEvent = typeof icalEvent === 'object' && (icalEvent.content || icalEvent.path || icalEvent.href || icalEvent.raw)
                ? { ...icalEvent } : { content: icalEvent };

            eventObject = {
                ...iEvent,
                contentType: 'application/ics',
                filename: iEvent.filename || 'invite.ics',
    ...
    getAlternatives() {
        ...
        if (this.mail.icalEvent) {
            const icalEvent = processAlt(icalEvent, this.mail.icalEvent, null);
        ...
        // 将所有替代内容合并到数组中
        const alternatives = Array.prototype.concat.call([], text, html, watchHtml, amp, eventObject, this.mail.alternatives)
    ...
    _prepareAttachmentObject(obj) {
        ...
        if (regexs.HTTP_URL.test(prepared.path)) {
            prepared.href = prepared.path;
