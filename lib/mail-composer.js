...
class MailComposer {
    ...
    getAttachments(findRelated = false) {
        ...
        const processedAttachments = attachments.map((attachment, i) => {
            // 处理dataURL格式
            const processedAttachment = regexs.DATA_URL.test(attachment.path || attachment.href)
                ? this._processDataUrl(attachment) : attachment;
            this._normalizeAttachmentPaths(processedAttachment); // 标准化路径并设置内容
            const { filename, path, href, cid, raw, encoding, headers, contentType: attachContentType } = processedAttachment;
            const source = path || href || 'bin';
            const contentType = attachContentType || detectMimeType(filename || source);
            const isImage = regexs.IMAGE_TYPE.test(contentType);
            const isMessageNode = regexs.MESSAGE_TYPE.test(contentType);
            const contentDisposition = processedAttachment.contentDisposition ||
                (isMessageNode || (isImage && cid) ? 'inline' : 'attachment');
            const contentTransferEncoding = 'contentTransferEncoding' in processedAttachment
                ? processedAttachment.contentTransferEncoding : isMessageNode ? '7bit' : 'base64';
    ...
    getAlternatives() {
        ...
        // 处理替代内容对象
        const processSimpleAlternative = (letValue, value, defaultContentType) => {
            typeof value === 'object' && (value.content || value.path || value.href || value.raw) ?
                letValue = value : { content: value };
            letValue.contentType = defaultContentType;
            return letValue;
        };

        // 处理纯文本内容
        if (this.mail.text) text = processSimpleAlternative(text, this.mail.text, 'text/plain; charset=utf-8');

        // 处理Apple Watch HTML内容
        if (this.mail.watchHtml) watchHtml = processSimpleAlternative(watchHtml, this.mail.watchHtml, 'text/watch-html; charset=utf-8');

        // 处理AMP HTML内容
        if (this.mail.amp) amp = processSimpleAlternative(amp, this.mail.amp, 'text/x-amp-html; charset=utf-8');

        // 处理日历事件 - 使用公共函数但传递空值
        if (this.mail.icalEvent) {
            icalEvent = processSimpleAlternative(icalEvent, this.mail.icalEvent, null); // 传递空值
        ...
        // 处理HTML内容
        if (this.mail.html) html = processSimpleAlternative(html, this.mail.html, 'text/html; charset=utf-8');

        // 将所有替代内容合并到数组中
        [].concat(text || [], watchHtml || [], amp || [], html || [], eventObject || [], this.mail.alternatives || [])
            .forEach(originalAlternative => {
                let alternative = { ...originalAlternative }; // 创建处理副本

                // 处理dataURL格式的内容
                if (regexs.DATA_URL.test(alternative.path || alternative.href))
                    alternative = { ...alternative, ...this._processDataUrl(alternative) };

                this._normalizeAttachmentPaths({ ...alternative });  // 标准化路径

                const { filename, path, href, contentTransferEncoding, raw, encoding, headers, contentType, content } = alternative;
                alternatives.push({
    ...
    _createContentNode(parentNode, element) {
        ...
        let encoding = (element.encoding || 'utf8').toString().toLowerCase().replace(regexs.ENCODING_FORMAT, '');
        ...
        if (element.cid) node.setHeader('Content-Id', '<' + element.cid.replace(regexs.CID_CLEAN, '') + '>');
        if (element.contentTransferEncoding) node.setHeader('Content-Transfer-Encoding', element.contentTransferEncoding);
        else if (this.mail.encoding && regexs.TEXT_TYPE.test(element.contentType))
            node.setHeader('Content-Transfer-Encoding', this.mail.encoding);

        // 设置内容处置方式
        if (!regexs.TEXT_TYPE.test(element.contentType) || element.contentDisposition)
            node.setHeader('Content-Disposition', element.contentDisposition
                || (element.cid && regexs.IMAGE_TYPE.test(element.contentType) ? 'inline' : 'attachment'));
