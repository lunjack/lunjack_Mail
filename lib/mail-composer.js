...
class MailComposer {
    ...
    compile() {
        ...
        ['from', 'sender', 'to', 'cc', 'bcc', 'reply-to', 'in-reply-to', 'references', 'subject', 'message-id', 'date']
            .forEach(header => {
                let key = header.replace(regexs.HYPHEN_REPLACE, (o, c) => c.toUpperCase());
    ...
    getAttachments(findRelated) {
        ...
        let attachments = [].concat(this.mail.attachments || []).map((attachment, i) => {
            // 处理dataURL格式的附件
            if (regexs.DATA_URL.test(attachment.path || attachment.href)) attachment = this._processDataUrl(attachment);

            // 检测内容类型
            let contentType = attachment.contentType || detectMimeType(attachment.filename || attachment.path
                || attachment.href || 'bin');
            let isImage = regexs.IMAGE_TYPE.test(contentType);
            let isMessageNode = regexs.MESSAGE_TYPE.test(contentType);
            // 确定内容处置方式：内联或附件
