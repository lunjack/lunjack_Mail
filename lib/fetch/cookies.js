...
class Cookies {
    ...
    set(cookieStr, url) {
        ...
        // 处理Cookie的域名
        if (domain) {
            // 不允许跨域Cookie(如果不匹配,则使用URL的主机名作为域名)
            if (!this._isDomainMatch(hostname, domain)) cookie.domain = hostname;
        }
        else cookie.domain = hostname; // 如果没有设置域名，使用URL的主机名

        // 如果没有设置路径，使用URL的路径
        if (!path) cookie.path = this.getPath(pathname);
    ...
    match(cookie, url) {
        ...
        // 检查主机名是否不匹配Cookie域名
        if (!this._isDomainMatch(urlObj.hostname, cookie.domain)) return false;

        let path = this.getPath(urlObj.pathname);                         // 检查路径是否匹配
        if (!path.startsWith(cookie.path)) return false;                  // 如果路径不以Cookie路径开头，则不匹配
        if (cookie.secure && urlObj.protocol !== 'https:') return false;  // 如果Cookie是安全的但URL不是https，则不匹配
