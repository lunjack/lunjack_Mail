        ...
        try {
            if (body instanceof Buffer) processedBody = body;
                ...
                processedBody = Buffer.from(
                    Object.entries(body)
                        .map(([key, value]) => `${encodeURIComponent(key)}=${encodeURIComponent(value.toString().trim())}`)
                        .join('&')
                );
...
// 处理重定向函数
function _handleRedirect(res, url, options, fetchRes, finishedFlag) {
...
function nmfetch(url, options = {}) {
    ...
    let cookies;
    let handler = protocol === 'https:' ? https : http;

    // 设置默认请求头
    let headers = {
        'accept-encoding': 'gzip,deflate',
        'user-agent': 'lunjack-mail/' + packageData.version
    };
    ...
    // 处理HTTPS协议的SNI
    if (protocol === 'https:' && hostname && hostname !== reqOptions.host && !net.isIP(hostname)
        && !reqOptions.servername) reqOptions.servername = hostname;
    ...
    req.on('response', res => {
        ...
        // 处理重定向
        if (_handleRedirect(res, url, options, fetchRes, finishedFlag)) return;
