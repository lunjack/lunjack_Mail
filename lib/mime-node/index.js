...
const { callbackPromise, _handleReadableStream, fs, nmfetch, regexs, resetRegex } = require('../shared');
...
class MimeNode {
    ...
    build(callback) {
        ...
        let returned = false;
        stream.on('readable', () => _handleReadableStream(stream, buf, buflen));
        function cleanup() {
            if (returned) return;
            returned = true;
        };
            ...
            if (content) {
                ...
                if (content._resolve) {
                    ...
                    sourceStream.on('error', err => callback(err))
                        .on('readable', () => _handleReadableStream(sourceStream, chunks, chunklen))
                        .on('end', () => {
                            cleanup();
