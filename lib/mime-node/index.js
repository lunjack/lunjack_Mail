...
class MimeNode {
    ...
    _getStream(content = {}) {
        ...
        const { _resolvedValue, pipe, path, href, httpHeaders } = content;
    ...
    _parseAddresses(addresses) {
        return [].concat.apply([], [].concat(addresses).map(mAddress => {
            ...
            if (mAddress?.address) {
                ...
                mAddress.address = this._normalizeAddress(address);
                mAddress.name = name || '';
                return [mAddress];
            }
    ...
    _normalizeHeaderKey(key = '') {
        const rL = resetRegex(regexs.LINEBREAKS);
        const rNHK = resetRegex(regexs.NORMALIZE_HEADER_KEY);
        key = key.toString().replace(rL, ' ').trim().toLowerCase() // 转换为字符串,删除换行符,行首和行尾空格并转换为小写
            .replace(rNHK, c => c.toUpperCase())                   // 使用大写单词,除了MIME
            .replace(regexs.CONTENT_FEATURES, 'Content-features'); // 特殊处理Content-features头部
    ...
    _encodeHeaderValue(key, value) {
        key = this._normalizeHeaderKey(key);
        const rL = resetRegex(regexs.LINEBREAKS);
        const rAB = resetRegex(regexs.ANGLE_BRACKET);
        const rBR = resetRegex(regexs.BLANK_REGEX);
    ...
    _normalizeAddress(address = '') {
        ...
        let user = address.substring(0, lIndex);
    ...
    _getTextEncoding(value = '') {
        ...
        const rNL = resetRegex(regexs.NON_LATIN);
        const rL = resetRegex(regexs.LATIN);
        const nonLatinLen = (value.match(rNL) || []).length;
        const latinLen = (value.match(rL) || []).length;
