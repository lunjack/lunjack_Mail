...
class MimeNode {
    ...
    _processHeadersOp(key, value, singleOp, isSet) {
        ...
        // set操作：先移除已存在的同名头部
        if (isSet) this._headers = this._headers.filter(item => item.key !== key);
    ...
    build(callback) {
        ...
        let buf = [], buflen = 0, returned = false;
    ...
    getTransferEncoding() {
        ...
        if (this.content) {
            ...
            if (!transferEncoding || ![B64, QP].includes(transferEncoding)) {
                if (regexs.TEXT_TYPE.test(contentType)) {
                    ...
                    // 否则，如果内容是字符串或Buffer，则使用QP或Base64
                        ...
                        transferEncoding = this._getTextEncoding(this.content) === 'Q' ? QP : B64;
                    // 否则,如果对于流，我们无法检查内容，因此使用首选编码或回退到QP
                    else transferEncoding = this.textEncoding === 'B' ? B64 : QP;
                }
                // 否则,如果不是 multipart 或 message 类型，默认使用 base64 编码
                else if (!regexs.MULTIPART_OR_MESSAGE.test(contentType)) transferEncoding = transferEncoding || B64;
            }
        }
        ...
        let sendContent = () => {
            if (this.content) {
                ...
                if (this.content._resolve) {
                    let chunks = [], chunklen = 0, returned = false;
