...
class MimeNode {
        ...
        this._headers.forEach(header => {
            ...
            switch (header.key) {
                ...
                case 'Content-Type':
                    ...
                    if (this.filename) {
                        // 为非兼容客户端（如QQ网页邮件）添加支持
                        // 我们不能使用buildHeaderValue构建值，因为该值是非标准的，
                        // 并且会被转换为我们不希望的参数延续编码
     ...
     */
    _encodeWords(value) {
        // 将encodeAll参数设置为true，尽管这违反RFC2047的建议，
        // 默认情况下只有包含非ascii的单词应转换为编码字
        // 但某些客户端（例如Zimbra）处理不当并删除周围的空格
    ...
    _getTextEncoding(value) {
        ...
        if (!encoding) {
            // 统计拉丁字母符号和8位范围符号+控制符号
            // 如果有更多拉丁字符，则使用quoted-printable
            // 编码，否则使用base64
