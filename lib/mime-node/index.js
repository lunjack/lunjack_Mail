...
const {
    detectMimeType, isPlainText, hasLongerLines, foldLines, parseHeaderValue, buildHeaderValue, encodeWord
} = require('../mime-funcs');
...
class MimeNode {
    constructor(contentType, options) {
        ...
        // 用于生成唯一边界值（前缀添加到共享基础值之前）
        this._nodeId = ++this.rootNode.nodeCounter;

        // 此节点的头部值列表，格式为[{ key: '', value: '' }]
        this._headers = [];

        // 如果内容仅使用ASCII可打印字符则为true
        this._isPlainText = false;

        // 如果内容是纯文本但有超过允许长度的行则为true
        this._hasLongLines = false;

        // 如果设置，则使用此值作为信封而不是生成一个
        this._envelope = false;

        // 如果设置，则使用此值作为流内容而不是构建它
        this._raw = false;

        // 在通过createReadStream暴露之前，消息将被管道传输的额外转换流
        this._transforms = [];

        // 在通过createReadStream暴露之前，消息将被管道传输的额外处理函数
        this._processFuncs = [];
            ...
            if (this.content) {
                ...
                let createStream = () => {
                    if (['quoted-printable', 'base64'].includes(transferEncoding)) {
                        contentStream = new (transferEncoding === 'base64' ? Base64Encoder : QpEncoder).Encoder(options);
    ...
    _normalizeHeaderKey(key) {
        key = (key || '')
            ...
            .replace(/^Content-Features$/i, 'Content-features');  // 特殊情况
    ...
    _handleContentType(structured) {
        ...
        this.multipart = /^multipart\//i.test(this.contentType) ? this.contentType.substring(this.contentType.indexOf('/') + 1) : false;
    ...
    _encodeAddressName(name) {
        if (!/^[\w ]*$/.test(name)) {
            if (/^[\x20-\x7e]*$/.test(name)) return '"' + name.replace(/([\\"])/g, '\\$1') + '"';
