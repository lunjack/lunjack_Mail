...
class MimeNode {
    ...
    //  统一处理头部操作，支持多种输入格式(singleOp传递函数)
    _processHeadersOp(key, value, singleOp) {
    ...
    setHeader(key, value) {
        // 如果已处理批量操作，直接返回
        if (this._processHeadersOp(key, value, this.setHeader)) return this;

        // 单个头部设置
        key = this._normalizeHeaderKey(key);
        this._headers = this._headers.filter(item => item.key !== key); // 过滤已存在的头部
    ...
    addHeader(key, value) {
        // 如果已处理批量操作，直接返回
        if (this._processHeadersOp(key, value, this.addHeader)) return this;

        // 单个头部添加
        key = this._normalizeHeaderKey(key);
        if (Array.isArray(value)) value.forEach(val => { this._headers.push({ key, value: val }); });
    ...
    _encodeHeaderValue(key, value) {
        ...
        switch (key) {
            ...
            case 'In-Reply-To':
            case 'Content-Id':
                value = (value || '').toString().replace(regexs.LINEBREAKS, ' ');
            ...
            case 'Content-Type':
            // 如果包含文件名，则已编码
            case 'Content-Disposition': return (value || '').toString().replace(regexs.LINEBREAKS, ' ');
