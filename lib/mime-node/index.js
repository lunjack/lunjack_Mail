...
class MimeNode {
    ...
    // 处理批量头部操作
    _processBulkHeaders(key, singleOp) {
    ...
    setHeader(key, value) {
        ...
        if (!value && key && typeof key === 'object') {
            this._processBulkHeaders(key, this.setHeader);
            return this;
        }

        key = this._normalizeHeaderKey(key);
        this._headers = this._headers.filter(item => item.key !== key);
    ...
    addHeader(key, value) {
        // 允许一次设置多个头部
        if (!value && key && typeof key === 'object') {
            this._processBulkHeaders(key, this.addHeader);
            return this;
        }
        ...
        if (Array.isArray(value)) {
            value.forEach(val => {
                this._headers.push({ key, value: val });
            });
            return this;
        }
