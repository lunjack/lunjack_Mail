...
class MimeNode {
    ...
    getTransferEncoding() {
        ...
        if (this.content) {
            ...
            if (!transferEncoding || !['base64', 'quoted-printable'].includes(transferEncoding)) {
                if (regexs.REGEX_TEXT_TYPE.test(contentType)) {
                    // 如果没有特殊符号，则无需修改文本
                ...
                else if (!regexs.REGEX_MULTIPART_OR_MESSAGE.test(contentType)) transferEncoding = transferEncoding || 'base64';
            }
        }
        ...
        this._headers.forEach(header => {
            ...
            switch (header.key) {
                ...
                case 'Content-Type':
                    ...
                    if (regexs.REGEX_TEXT_PLAIN.test(structured.value) && typeof this.content === 'string' && regexs.REGEX_NON_ASCII.test(this.content))
                    ...
                    if (this.filename) {
                        ...
                        if (param !== this.filename || regexs.REGEX_SPECIAL_CHARS.test(param)) param = '"' + param + '"';
    ...
    _normalizeHeaderKey(key) {
        ...
        key = (key || '')
            ...
            .replace(regexs.REGEX_CONTENT_FEATURES, 'Content-features');  // 特殊情况
    ...
    _handleContentType(structured) {
        ...
        this.multipart = regexs.REGEX_MULTIPART.test(this.contentType) ? this.contentType.substring(this.contentType.indexOf('/') + 1) : false;
    ...
    _encodeHeaderValue(key, value) {
        ...
        resetRegex(regexs.REGEX_CRLF);
        resetRegex(regexs.REGEX_ANGLE_BRACKET);
        switch (key) {
            // 结构化头部
            case 'From':
            case 'Sender':
                    ...
                    .apply(
                        ...
                        [].concat(value || '').map(elm => {
                            ...
                            elm = (elm || '')
                                ...
                                .trim();
                            return elm.replace(regexs.REGEX_ANGLE_BRACKET, str => str.replace(regexs.REGEX_WHITESPACE, '')).split(regexs.REGEX_WHITESPACE);
                        })
                    )
                    .map(elm => {
    ...
    _normalizeAddress(address) {
        resetRegex(regexs.REGEX_FORBIDDEN_CHARS);
        address = (address || '')
            .toString()
            .replace(regexs.REGEX_FORBIDDEN_CHARS, ' ') // 移除不允许的字符
            .trim();
    ...
    _encodeAddressName(name) {
        if (!regexs.REGEX_ALPHANUMERIC_UNDERSCORE_SPACE.test(name)) {
            resetRegex(regexs.REGEX_QUOTED_PAIR);
            if (regexs.REGEX_PRINTABLE_ASCII.test(name)) return '"' + name.replace(regexs.REGEX_QUOTED_PAIR, '\\$1') + '"';
    ...
    _getTextEncoding(value) {
        ...
        if (!encoding) {
            resetRegex(regexs.REGEX_NON_LATIN);
            resetRegex(regexs.REGEX_LATIN);
            nonLatinLen = (value.match(regexs.REGEX_NON_LATIN) || []).length;
            latinLen = (value.match(regexs.REGEX_LATIN) || []).length;
