...
const { callbackPromise, handleReadableStream, fs, nmfetch, regexs, resetRegex } = require('../shared');
const {
...
class MimeNode {
    constructor(contentType, options = {}) {
        ...
        this.normalizeHeaderKey = normalizeHeaderKey;
        this.disableFileAccess = !!disableFileAccess;
        this.disableUrlAccess = !!disableUrlAccess;
        this.keepBcc = !!keepBcc;                       // Bcc默认不保留
        this.rootNode = rootNode || this;               // 当前MIME树的根节点
    ...
    replace(node) {
        ...
        this.parentNode.childNodes.forEach((childNode, i) => {
            if (childNode === this) {
                ...
                this.rootNode = this, this.parentNode = undefined;
    ...
    build(callback) {
        ...
        const buf = [], buflen = { value: 0 };
        let returned = false;
        stream.on('readable', () => handleReadableStream(stream, buf, buflen));
        function cleanup() {
            if (returned) return;
            returned = true;
        };
        ...
        stream.once('error', err => {
            ...
            cleanup();
            // 如果流没有返回任何内容，则返回空缓冲区
            if (chunk?.length) buf.push(chunk), buflen.value += chunk.length;
    ...
    stream(outputStream, options, done) {
            ...
            if (content) {
                ...
                if (content._resolve) {
                    ...
                    sourceStream.on('error', err => callback(err))
                        .on('readable', () => handleReadableStream(sourceStream, chunks, chunklen))
                        .on('end', () => {
                            cleanup();
        ...
        if (this._raw) {
            setImmediate(() => {
                ...
                raw.on('end', finalize).on('error', err => outputStream.emit('error', err));
