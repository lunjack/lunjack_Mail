...
const { callbackPromise, fs, nmfetch, regexs, resetRegex } = require('../shared');
...
class MimeNode {
     ...
     * @param {Buffer[]} chunkArray 存储数据块的数组
     * @param {{value: number}} lengthRef 包含value属性的对象，用于累加数据长度
     */
    _handleReadableStream(stream, chunkArray, lengthRef) {
        let chunk;
        while ((chunk = stream.read()) !== null) {
            chunkArray.push(chunk);
            lengthRef.value += chunk.length;
        }
    }
    ...
    build(callback) {
        ...
        const buf = [];
        let buflen = { value: 0 }, returned = false;
        stream.on('readable', () => this._handleReadableStream(stream, buf, buflen));
        function cleanup() {
            if (returned) return;
            returned = true;
        };
        ...
        stream.once('end', chunk => {
            ...
            if (chunk && chunk.length) {
                buf.push(chunk);
                buflen.value += chunk.length;
            }
            return callback(null, Buffer.concat(buf, buflen.value));
        });
    ...
    getTransferEncoding() {
        ...
        if (content) {
            tEncoding = (this.getHeader('Content-Transfer-Encoding') || '').toString().toLowerCase().trim();
            if (!tEncoding || ![B64, QP].includes(tEncoding)) {
                if (regexs.TEXT_TYPE.test(contentType)) {
                    if (this._isPlainText && !this._hasLongLines) tEncoding = '7bit';  // 如果没有特殊符号，则无需修改文本
                    else if (typeof content === 'string' || content instanceof Buffer) // 否则,内容是字符串或Buffer,则用QP或Base64
                        tEncoding = this._getTextEncoding(content) === 'Q' ? QP : B64;
                    else tEncoding = this.textEncoding === 'B' ? B64 : QP; // 否则,如果对于流,因无法检查内容,则使用首选编码或回退到QP
                }
                // 否则,如果不是 multipart 或 message 类型，默认使用 base64 编码
                else if (!regexs.MULTIPART_OR_MESSAGE.test(contentType)) tEncoding = tEncoding || B64;
            }
        }
        return tEncoding;
    }
     ...
     */
    buildHeaders() {
        const tEncoding = this.getTransferEncoding();
        const C_DISPOSITION = 'Content-Disposition', DATE = 'Date', M_VERSION = 'MIME-Version', C_TYPE = 'Content-Type';
        const headers = [], { filename, content } = this;
        ...
        this._headers.forEach(header => {
            ...
            if (options.prepared) {
                ...
                // 头部值已准备就绪
                options.foldLines ? headers.push(foldLines(pushKV)) : headers.push(pushKV);
                return;
            }
    ...
    createReadStream(options = {}) {
        ...
        if (this.newline) {
            ...
            const nTransform = winbreak ? new LeWindows() : new LeUnix();
            const newStream = outputStream.pipe(nTransform);
    ...
    stream(outputStream, options, done) {
        ...
        let returned = false;  // 保护实际回调防止多次触发
        function cleanup() {
            if (returned) return;
            returned = true;
        };
        ...
        const sendContent = () => {
            ...
            if (content) {
                ...
                if (typeof content.pipe === 'function') {
                    ...
                    content.once('error', this._contentErrorHandler);
                }

                const createStream = () => {
                    if ([QP, B64].includes(transferEncoding)) {
                        contentStream = new (transferEncoding === B64 ? Base64Encoder : QpEncoder)(options);
                        contentStream.pipe(outputStream, { end: false }).once('end', finalize).once('error', err => callback(err));

                        localStream = this._getStream(content);
                        localStream.pipe(contentStream);
                    } else {
                        // 任何不是QP或Base54的内容都按原样传递
                        localStream = this._getStream(content);
                        localStream.pipe(outputStream, { end: false }).once('end', finalize);
                    }
                ...
                if (content._resolve) {
                    let chunklen = { value: 0 };
                    returned = false;
                    const chunks = [];
                    const sourceStream = this._getStream(content);
                    sourceStream.on('error', err => callback(err))
                        .on('readable', () => this._handleReadableStream(sourceStream, chunks, chunklen))
                        .on('end', () => {
                            cleanup();
                            content._resolve = false;
                            content._resolvedValue = Buffer.concat(chunks, chunklen.value);
        ...
        if (this._raw) {
            setImmediate(() => {
                ...
                this._getStream(this._raw).pipe(outputStream, { end: false }).once('end', finalize)
 ...
 */
    _getStream(content) {
        ...
        // 创建并返回包含指定数据的流
        const createContentStream = (data) => {
        ...
        // 将字符串或缓冲区内容作为流传递
        if (content._resolvedValue) { return createContentStream(content._resolvedValue); }
        // 否则，如果内容是流，则直接返回内容
        else if (typeof content.pipe === 'function') return content;
        // 否则，如果内容是文件路径，则返回文件流
        else if (content && typeof content.path === 'string' && !content.href) {
            if (this.disableFileAccess) return createErrorStream('文件访问被拒绝：' + content.path);
            return fs.createReadStream(content.path);                       // 读取文件
        }
        // 否则，如果内容是URL，则返回URL流
        else if (content && typeof content.href === 'string') {
            if (this.disableUrlAccess) return createErrorStream('URL访问被拒绝：' + content.href);
            return nmfetch(content.href, { headers: content.httpHeaders }); // 获取URL
        }
        // 否则，将字符串或缓冲区内容作为流传递
        else return createContentStream(content || '');
    }
    ...
    _normalizeHeaderKey(key) {
        resetRegex(regexs.LINEBREAKS);
        resetRegex(regexs.NORMALIZE_HEADER_KEY);
        key = (key || '')
            .toString()
            .replace(regexs.LINEBREAKS, ' ') // 键中不能有换行符
            .trim()
            .toLowerCase()
            // 使用大写单词，除了MIME
            .replace(regexs.NORMALIZE_HEADER_KEY, c => c.toUpperCase())
  ...
  */
    _encodeHeaderValue(key, value) {
        ...
        resetRegex(regexs.GLOBAL_WHITESPACE);

        const cleanValue = (val) => {
            return (val || '').toString().replace(regexs.LINEBREAKS, ' '); // 将值转换为字符串，并替换掉换行符
        };

        const ensureAngleBrackets = (val) => {
        ...
        switch (key) {
            ...
            case 'Content-Id':
                ...
                value = this._encodeWords(value);
                value = ensureAngleBrackets(value);
                return value;
            // 用<>括起来的值，以空格分隔的列表
            case 'References':
                value = [].concat(value || '')
                    .flatMap(ref =>
                        cleanValue(ref).trim()
                            .replace(regexs.ANGLE_BRACKET, m => m.replace(regexs.WHITESPACE, ''))
                            .split(regexs.WHITESPACE) // 将其中的尖括号内的空白字符删除,然后按空白字符分割
                    )
            ...
            case 'Content-Type':   // 如果包含文件名，则已编码
            case 'Content-Disposition': return cleanValue(value);
            // 默认encodeWords仅在需要时编码，否则返回原始字符串
            default: return this._encodeWords(cleanValue(value));
        }
    }
    ...
    _convertAddresses(addresses, uniqueList = []) {
        ...
        [].concat(addresses || []).forEach(addrObj => {
            ...
            if (address) {
                ...
                if (!name) values.push(normAddr.indexOf(' ') >= 0 ? `<${normAddr}>` : normAddr);
                ...
                let groupListAddresses = (group.length ? this._convertAddresses(group, uniqueList) : '').trim();
    ...
    _normalizeAddress(address) {
        ...
        address = (address || '').toString().replace(regexs.FORBIDDEN_CHARS, ' ').trim();
    ...
    _getTextEncoding(value) {
        ...
        const nonLatinLen = (value.match(regexs.NON_LATIN) || []).length;
        const latinLen = (value.match(regexs.LATIN) || []).length;
     ...
     */
    _generateMessageId() {
        // 使用不同长度的随机字节生成类似UUID的格式
        const randomPart = [4, 2, 2, 2, 6].map(len => randomBytes(len).toString('hex')).join('-');
        // 优先使用发件人地址中的域名，服务器主机名或localhost次之
        const domain = (this.getEnvelope().from || this.hostname || 'localhost').split('@').pop();
