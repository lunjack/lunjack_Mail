...
function httpProxyClient(proxyUrl, destinationPort, destinationHost, callback) {
    ...
    const state = { finished: false }; // 初始连接的错误处理:一旦连接建立,错误处理将传递给socket使用者
    ...
    const socket = connect(options, () => {
        ...
        function onSocketData(chunk) {
            ...
            const headers = Buffer.concat(headerChunks).toString('binary');

            const headersEndIndex = headers.indexOf('\r\n\r\n');
            if (headersEndIndex !== -1) {
                ...
                const statusMatch = headerPart.match(regexs.HTTP_RESPONSE_STATUS_LINE);
                ...
                socket.removeListener('error', tempSocketErr).removeListener('timeout', timeoutErr)
                    .setTimeout(0), headerChunks = [];          // 禁用超时,成功时重置 headerChunks
    ...
    socket.setTimeout(httpProxyClient.timeout || 30 * 1000)      // 设置连接超时
        .on('timeout', timeoutErr).once('error', tempSocketErr); // 监听连接错误
}
