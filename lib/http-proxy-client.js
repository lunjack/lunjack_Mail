...
function httpProxyClient(proxyUrl, destinationPort, destinationHost, callback) {
    let proxy, options, connect, socket; // 创建到代理服务器的socket连接
    try {
    ...
    if (proxy.protocol === 'https:') {
        // 对于HTTPS代理，可以使用不受信任的代理，只要验证实际的SMTP证书
    ...
    // 初始连接的错误处理。一旦连接建立，错误处理将传递给socket使用者
    let finished = false;
    ...
    socket.once('error', tempSocketErr); // 监听连接错误
}
