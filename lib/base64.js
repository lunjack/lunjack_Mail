...
function base64Wrap(str = '', lineLength = 76) {
    ...
    while (pos < str.length) {
        ...
        const wrappedLines = str.substring(pos, end).replace(new RegExp(`.{${lineLength}}`, 'g'), '$&\r\n');
        result.push(wrappedLines), pos = end;
    }
...
class Base64Encoder extends Transform {
    ...
    _transform(chunk, encoding, done) {
        ...
        if (remain) {
            ...
            this._remainingBytes = chunk.slice(chunkEnd);
            chunk = chunk.slice(0, chunkEnd);
        }
