...
function base64Wrap(str = '', lineLength = 76) {
    ...
    while (pos < str.length) {
        const wrappedLines = str.substring(pos, pos + chunkLength).replace(new RegExp(`.{${lineLength}}`, 'g'), '$&\r\n');
        result.push(wrappedLines), pos += chunkLength;
    }
...
class Base64Encoder extends Transform {
    constructor(options = {}) {
        ...
        this._curLine = '', this._remainingBytes = false; // 当前行内容,上次剩余的字节
        this.inputBytes = 0, this.outputBytes = 0;
    }
    ...
    _transform(chunk, encoding, done) {
        ...
        if (rLen) {
            chunk = Buffer.concat([this._remainingBytes, chunk], rLen + chunk.length);
        ...
        if (remain) {
            this._remainingBytes = chunk.slice(chunk.length - remain);
            chunk = chunk.slice(0, chunk.length - remain);
        }
        ...
        if (this.options.lineLength) {
            ...
            if (indexLF < 0) this._curLine = b64, b64 = '';
                ...
                this._curLine = b64.substring(indexLF + 1), b64 = b64.substring(0, indexLF + 1);
                if (b64 && !b64.endsWith('\r\n')) b64 += '\r\n'; // 确保以换行符结尾
            }
        }
