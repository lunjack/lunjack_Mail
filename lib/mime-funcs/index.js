 ...
 */
function encodeWord(data, mimeWordEncoding, maxLength = 0) {
    const newMWE = (mimeWordEncoding || 'Q').toString().toUpperCase().trim().charAt(0);
    ...
    if (newMWE === 'Q') {
        const NSC = resetRegex(regexs.NON_SAFE_CHAR);
        encodedStr = qpEncode(data).replace(NSC, chr => {
            const ord = chr.charCodeAt(0).toString(16).toUpperCase();
            const XX = ord.length === 1 ? `0${ord}` : ord
    ...
    // 返回编码后的字符串
    return `=?${toCharset}?${newMWE}?${encodedStr}${encodedStr.slice(-2) === '?=' ? '' : '?='}`
}
 ...
 */
function encodeWords(value, mimeWordEncoding, maxLength = 0, encodeAll) {
    // 查找第一个包含不可打印ASCII或特殊符号的单词
    const firstMatch = value.match(regexs.FIRST_NON_ASCII_WORD); // 查找第一个包含不可打印ASCII的单词
    if (!firstMatch) return value;                               // 如果第一个包含为假，则返回原始值

    const lastMatch = value.match(regexs.LAST_NON_ASCII_WORD);  // 同上(匹配最后)...
    ...
    const startIndex = firstMatch.index + (firstMatch[0].match(regexs.NON_WHITESPACE) || { index: 0 }).index;
    const endIndex = lastMatch.index + (lastMatch[1] || '').length;

    const startStr = startIndex ? value.substring(0, startIndex) : ''
    const endStr = endIndex < value.length ? value.substring(endIndex) : '';
    const eWord = encodeWords(value.substring(startIndex, endIndex), mimeWordEncoding || 'Q', maxLength);
...
function parseHeaderValue(str) {
    ...
    // 解析状态管理：
    let state = 'value';    // 当前解析状态：'value'=主值 | 'key'=参数键 | 'quoted'=引号内
    let currentKey = '';    // 当前正在解析的参数键名
    let currentValue = '';  // 当前正在解析的值（可能是主值或参数值）
    let quoteChar = '';     // 当前引号字符（'"' 或 "'"）
    ...
    for (let i = 0; i < str.length; i++) {
        ...
        switch (state) {
            case 'value':  // 解析主值部分（如：text/plain）
                if (char === ';') {
                    result.value = currentValue.trim();  // 保存主值（去除前后空格）
                    currentValue = '';                   // 重置当前值
                    state = 'key';                       // 切换到键解析
                }
...
function _processRFC2231Params(params) {
    ...
    Object.keys(params).forEach(key => {
        ...
        if ((match = key.match(regexs.RFC2231_KEY))) {
            const actualKey = key.substring(0, match.index);
...
function detectExtension(mimeType) {
    ...
    const rootType = parts.shift().trim();   // 主类型（如：application、text等）
    const subType = parts.join('/').trim();  // 子类型

    // 构建完整的MIME类型并查找对应的扩展名
    const fullMimeType = `${rootType}/${subType}`;
...
function _buildHeaderParam(key, data, maxLength = 50) {
    ...
    if (isPlainText(data, true)) {
        // 检查是否需要转换
        if (encodedStr.length <= maxLength) return [{ key, value: encodedStr }];
    ...
    // 否则，如果字符串包含Unicode或特殊字符，则将其拆分为多个块
        ...
        // 如果最后一行不为空，则将其推送到列表中
        if (line) list.push({ line, encoded });
    }
...
function _safeEncodeURIComponent(str = '') {
    ...
    try {
        ...
        const NSU = resetRegex(regexs.NON_SAFE_URI);
        return str.replace(NSU, '');  // 返回无效字符的替换字符串
    }

    const NSUC = resetRegex(regexs.NON_SAFE_URI_CHAR);
    // 确保encodeURIComponent未处理的字符也被转换
    return str.replace(NSUC, chr => _encodeURICharComponent(chr));
}
