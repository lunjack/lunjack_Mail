...
function isPlainText(value, isParam) {
    // 如果是参数，则允许双引号，否则不允许
    const re = isParam ? regexs.NON_PRINTABLE_AND_DOUBLE_QUOTE : regexs.NON_PRINTABLE;
...
function encodeWord(data, mimeWordEncoding, maxLength) {
    ...
    if (maxLength && (mimeWordEncoding !== 'B' ? encodedStr : base64Encode(data)).length > maxLength) {
        if (mimeWordEncoding === 'Q')
...
function buildHeaderValue(structured) {
    ...
    Object.keys(structured.params || {}).forEach(param => {
        ...
        if (!isPlainText(value, true) || value.length >= 75) {
            _buildHeaderParam(param, value, 50).forEach(encodedParam => {
                // 如果参数值安全 或 参数是编码后的字符串,则不编码
                if (!regexs.UNSAFE_HEADER_PARAM.test(encodedParam.value) || encodedParam.key.slice(-1) === '*')
        ...
        else if (regexs.UNSAFE_PARAM_VALUE.test(value)) paramsArray.push(param + '=' + JSON.stringify(value));
