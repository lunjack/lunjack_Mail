...
module.exports = {
    ...
    encodeWord(data, mimeWordEncoding, maxLength) {
        ...
        if (maxLength && (mimeWordEncoding !== 'B' ? encodedStr : base64.encode(data)).length > maxLength) {
            if (mimeWordEncoding === 'Q')
                ...
                if (lpart) parts.push(base64.encode(lpart)); // 如果最后一个字符串不为空，则将其推送到数组中
        ...
        // 否则，如果mimeWordEncoding为B，则使用base64编码
    ...
    buildHeaderParam(key, data, maxLength) {
        ...
        // 否则，如果字符串包含Unicode或特殊字符，则将其拆分为多个块
            ...
            for (i = 0, len = encodedStr.length; i < len; i++) {
                ...
                if (encoded) chr = this.safeEncodeURIComponent(chr); // 如果需要编码，则对当前字符进行URL编码
                else {
                    // 尝试对当前字符进行URL编码
        ...
        for (let i = 0, len = str.length; i < len; i++) {
            ...
            if (type === 'key') {
                ...
                // 如果当前不在引号内，且遇到分号，则将值添加到参数中，并重置值
                    ...
                    if (key === false) response.value = value.trim(); // 如果没有键名，则去空格将值添加到值中
                    else response.params[key] = value.trim(); // 否则，将值添加到参数中
    ...
    /**
     * 返回内容类型字符串的文件扩展名。如果找不到合适的扩展名，
     * 则使用'bin'作为默认扩展名
     *
     * @param {String} mimeType 要检查的内容类型
     * @return {String} 文件扩展名
     */
    detectExtension: mimeType => mimeTypes.detectExtension(mimeType),

    /**
     * 返回文件扩展名的内容类型。如果找不到合适的内容类型，
     * 则使用'application/octet-stream'作为默认内容类型
     *
     * @param {String} extension 要检查的扩展名
     * @return {String} 文件扩展名
     */
