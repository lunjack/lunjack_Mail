...
function encodeWords(value, mimeWordEncoding, maxLength, encodeAll) {
    ...
    let startIndex = firstMatch.index + (firstMatch[0].match(regexs.NON_WHITESPACE) || { index: 0 }).index;
    let endIndex = lastMatch.index + (lastMatch[1] || '').length;

    encodedValue = (startIndex ? value.substring(0, startIndex) : '') +
    ...
    Object.keys(structured.params || {}).forEach(param => {
        ...
        if (!isPlainText(pValue, true) || pValue.length >= 75) {
            _buildHeaderParam(param, pValue, 50).forEach(encodedParam => {
                ...
                if (!regexs.UNSAFE_HEADER_PARAM.test(value) || key.slice(-1) === '*') paramsArray.push(key + '=' + value);
                // 否则将参数编码为JSON字符串
...
function parseHeaderValue(str) {
    const result = {
        value: '',      // 主要的值部分（如：text/plain）
        params: {}      // 参数字典（如：{charset: 'UTF-8'}）
    };

    // 输入验证：如果输入为空或不是字符串，直接返回空结果
...
function detectMimeType(filename) {
    ...
    let parsed = path.parse(filename);     // 解析文件路径获取扩展名
    ...
    if (Array.isArray(value)) return value[0]; // 如果值是数组，返回第一个元素
