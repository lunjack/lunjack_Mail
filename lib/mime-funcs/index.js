    ...
    // 否则，如果字符串包含Unicode或特殊字符，则将其拆分为多个块
        ...
        if (regexs.UTF16_HIGH_SURROGATE.test(encodedStr)) {
            ...
            for (i = 0, len = encodedStr.length; i < len; i++) {
                ...
                if (ord >= 0xd800 && ord <= 0xdbff && i < len - 1) {
                    chr += encodedStr.charAt(i + 1), encodedStrArr.push(chr);
                    i++;
                }
        ...
        for (i = 0; i < encodedStr.length; i++) {
            ...
            if ((line + chr).length >= maxLength) {
                ...
                // 如果字符串不包含Unicode或特殊字符，则不需要编码;否则需要编码并添加到行中
                !chr === encodedStr[i] ? encoded = true : encoded = false, startPos = i - 1;
            }
...
function _safeEncodeURIComponent(str = '') {
    ...
    return str.replace(rNSUC, chr => _encodeURICharComponent(chr));
}
