 ...
 *
 * @param {String} str 要检查的字符串(不测试超过128kB的字符串)
...
function encodeWords(value, mimeWordEncoding = "Q", maxLength = 0, encodeAll) {
    ...
    const eWord = encodeWords(value.substring(startIndex, endIndex), mimeWordEncoding, maxLength);
        ...
        if (regexs.UTF16_HIGH_SURROGATE.test(encodedStr)) {
            ...
            for (i = 0, len = encodedStr.length; i < len; i++) {
                ...
                if (ord >= 0xd800 && ord <= 0xdbff && i < len - 1)
                    chr += encodedStr.charAt(i + 1), encodedStrArr.push(chr), i++;
