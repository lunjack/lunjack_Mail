...
function _handleAddress(tokens) {
    ...
    for (let i = 0, len = tokens.length; i < len; i++) {
        ...
        if (token.type === 'operator') {
            ...
            if (prevToken && prevToken.noBreak && data[state].length) data[state][data[state].length - 1] += token.value;
            else data[state].push(token.value); // 否则作为新元素添加
        }
    }

    // 如果没有文本但有注释，用注释替换文本
    _replaceTextWithComment(data);
...
class Tokenizer {
    ...
    _processChar(chr, nextChr) {
        // 检查是否是期望的结束操作符
        if (!this.escaped && chr === this.operatorExpecting) {
        ...
        // 检查转义字符（在引号或注释中）
        if (!this.escaped && ['"', "'"].includes(this.operatorExpecting) && chr === '\\') {
            this.escaped = true;
            return;
        }

        // 处理文本字符
        this._addTextChar(chr);
        this.escaped = false;
    }
...
function addressparser(str, options) {
    ...
    if (options.flatten) {
        ...
        const walkAddressList = function (list) {
            list.forEach(address => {
                if (address.group) walkAddressList(address.group);
                else flattenedAddresses.push(address);
            });
        };
