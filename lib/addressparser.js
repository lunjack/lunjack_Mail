...
function _extractEmailFromText(data) {
    ...
    for (let i = text.length - 1; i >= 0; i--) {
        ...
        if (!foundAddress) {
            text[i] = currentText.replace(regexs.EMAIL_LOOSE, _regexHandler).trim();
...
function _handleAddress(tokens) {
    ...
    const { text, group, address } = data;
    // 如果组存在，处理组地址：组名和组成员列表
    if (isGroup)
        return [{
            name: text.join(' ') || '',
            group: group.length ? addressparser(group.join(',')) : []
        }];
    else {
        // 处理单个地址：从文本中检测电子邮件地址
        if (!address.length && text.length) _extractEmailFromText(data);
        _replaceTextWithComment(data);  // 再次检查是否需要将注释转换为文本

        const { text: newTxt, address: newAdd } = data; // 解钩处理后的数据
        // 如果找到多个地址，只保留第一个，其余的转到文本数组
        if (newAdd.length > 1) newTxt.push(...newAdd.splice(1));

        // 合并文本和地址数据
        const addressText = newTxt.join(' ').trim(), addressValue = newAdd.join(' ').trim();
        ...
        if (addressArr.address === addressArr.name)
            addressArr.address.match(/@/) ? addressArr.name = '' : addressArr.address = ''; // 如果是邮件地址,清除名称,否则,清除地址
...
class Tokenizer {
    constructor(str = '') {
        ...
        this.node = null, this.escaped = false, this.list = [];
    ...
    _processChar(chr, nextChr) {
        ...
        if (!this.escaped && !this.operatorExpecting && chr in this.operators) {
            this._addOperatorToken(chr, nextChr), this.operatorExpecting = this.operators[chr];
            return;
        }
    ...
    _addOperatorToken(chr, nextChr) {
        ...
        this.list.push(node), this.node = null;
    }
    ...
    _addTextChar(chr) {
        // 如果没有当前文本节点，则创建一个新的
        if (!this.node) this.node = { type: 'text', value: '' }, this.list.push(this.node);
...
function addressparser(str, options = {}) {
    ...
    const tokens = tokenizer.tokenize(), addresses = [];
    let currentAddress = [];
