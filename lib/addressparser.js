...
const { regexs } = require('lunjack-mail/lib/regexs');
...
function _handleAddress(tokens) {
    ...
    if (isGroup)
        ...
        // 创建地址对象
        const addressArr = { address: addressValue || addressText || '', name: addressText || addressValue || '' };
...
class Tokenizer {
    constructor(str = '') {
        ...
        this.operatorExpecting = ''; // 记录当前期望的结束操作符
    ...
    _processChar(chr, nextChr) {
        ...
        // 检查是否是新的开始操作符
        else if (!this.escaped && !this.operatorExpecting && chr in this.operators)
            return this._addOperatorToken(chr, nextChr, this.operators[chr]);

        // 检查转义字符（在引号或注释中）
        else if (!this.escaped && ['"', "'"].includes(this.operatorExpecting) && chr === '\\') return this.escaped = true;
