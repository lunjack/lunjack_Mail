...
const regexs = {
    ...
    REGEX_TEXT_TYPE: /^text\//i, // 匹配所有text/*内容类型
    REGEX_MULTIPART: /^multipart\//i, // 匹配multipart/*内容类型
    REGEX_CONTENT_FEATURES: /^Content-Features$/i, // 匹配Content-Features头部
    REGEX_MULTIPART_OR_MESSAGE: /^(multipart|message)\//i, // 匹配multipart/*或message/*内容类型

    REGEX_PRINTABLE_ASCII: /^[\x20-\x7e]*$/, // 匹配可打印ASCII字符
    REGEX_ALPHANUMERIC_UNDERSCORE_SPACE: /^[\w ]*$/, // 匹配仅包含字母数字、下划线和空格的字符串

    REGEX_TEXT_PLAIN: /^text\/plain\b/, // 匹配text/plain内容类型
    REGEX_NON_ASCII: /[\u0080-\uFFFF]/, // 匹配非ASCII字符（Unicode字符）
    REGEX_WHITESPACE: /\s+/, // 匹配空白字符
    REGEX_SPECIAL_CHARS: /[\s'"\\;:/=(),<>@[\]?]|^-/, // 匹配需要引号包裹的特殊字符

    REGEX_CRLF: /\r?\n|\r/g, // 匹配各种换行符（CRLF、LF、CR）
    REGEX_QUOTED_PAIR: /([\\"])/g, // 匹配需要转义的字符（反斜杠和引号）
