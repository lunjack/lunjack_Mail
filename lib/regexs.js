...
const regexs = {
    ...
    NON_PRINTABLE: /[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]|[^\x00-\x7F]/,      // 匹配不可打印ASCII字符
    NEED_ENCODING: /[\x00-\x20\x2B\x3D\x7F]|[^\x21-\x7E]/,  // 不可打印ASCII字符（包括非ASCII字符）
    FIRST_NON_ASCII_WORD: /(?:^|\s)([^\s]*["\u0080-\uFFFF])/,     // 匹配第一个包含非ASCII字符的单词
    NON_PRINTABLE_AND_DOUBLE_QUOTE: /[^\x09\x0a\x0d\x20-\x7e]|"/, // 匹配不可打印ASCII字符或双引号（用于参数检查）
    ...
    RESPONSE_550: /^550 /,                                   // 匹配550响应码
    RESPONSE_553: /^553 /,                                   // 匹配553响应码
    DATA_RESPONSE: /^[23]/,                                  // 匹配DATA命令的2xx或3xx响应码
    AUTH_SUCCESS: /^235\s+/,                                 // 匹配认证成功的235响应码
    AUTH_RESPONSE: /^334[ -]/,                               // 匹配AUTH认证的334响应码
    START_WITH_DIGITS: /^\d+/,                               // 匹配以数字开头的字符串
    CONTINUATION_RESPONSE: /^\d+-/,                          // 匹配多行响应的继续标记
