...
class SmtpConnection extends EventEmitter {
    ...
    connect(connectCallback) {
        ...
        // 如果secureConnection为true，则使用TLS连接
            ...
            if (this.options.tls)
                Object.keys(this.options.tls).forEach(key => { opts[key] = this.options.tls[key]; })
    ...
    */
    ...
    _onData(chunk) {
        ...
        for (let i = 0, len = lines.length; i < len; i++) {
            if (this._responseQueue.length) {
                ...
                if (regexs.CONTINUATION_RESPONSE.test(lastline.split('\n').pop())) {
    ...
    _formatError(message, type, response, command) {
        ...
        let responseCode = (typeof response === 'string' && Number((response.match(regexs.START_WITH_DIGITS) || [])[0])) || false;
    ...
    _onClose() {
        ...
        else if (regexs.RESPONSE_4XX_5XX.test(serverResponse))
            return this._onError(new Error('连接意外关闭'), 'ECONNECTION', serverResponse, 'CONN');
    ...
    _setEnvelope(envelope, callback) {
        ...
        if (this._envelope.from && regexs.INVALID_ADDRESS_CHARS.test(this._envelope.from))
        ...
        if (regexs.NON_ASCII.test(this._envelope.from)) useSmtpUtf8 = true;

        for (let i = 0, len = this._envelope.to.length; i < len; i++) {
            if (!this._envelope.to[i] || regexs.INVALID_ADDRESS_CHARS.test(this._envelope.to[i]))
            ...
            if (regexs.NON_ASCII.test(this._envelope.to[i])) useSmtpUtf8 = true;
        }
    ...
    _actionAUTH_CRAM_MD5_PASS(str, callback) {
        if (!str.match(regexs.AUTH_SUCCESS)) return callback(this._formatError('在等待"235"时登录序列无效', 'EAUTH', str, 'AUTH CRAM-MD5'));
    ...
    _actionMAIL(str, callback) {
        ...
        if (Number(str.charAt(0)) !== 2) {
            if (this._usingSmtpUtf8 && regexs.RESPONSE_550.test(str) && regexs.NON_ASCII.test(this._envelope.from))
                message = '不允许国际化的邮箱名称';
            else message = '邮件命令失败';
    ...
    _actionRCPT(str, callback) {
        ...
        if (Number(str.charAt(0)) !== 2) {
            // 这是一个软错误
            if (this._usingSmtpUtf8 && regexs.RESPONSE_553.test(str) && regexs.NON_ASCII.test(curRecipient)) message = '不允许国际化的邮箱名称';
    ...
    _getHostname() {
        ...
        // IP地址应包含在[]中
        if (defaultHostname.match(regexs.IPV4_ADDRESS)) defaultHostname = '[' + defaultHostname + ']';
