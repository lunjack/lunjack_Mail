...
class SmtpConnection extends EventEmitter {
    ...
    */
    ...
    _setEnvelope(envelope, callback) {
        ...
        if (regexs.NON_ASCII_CHARS.test(this._envelope.from)) useSmtpUtf8 = true;
        ...
        for (let i = 0, len = this._envelope.to.length; i < len; i++) {
            ...
            if (regexs.NON_ASCII_CHARS.test(this._envelope.to[i])) useSmtpUtf8 = true;
        }
    ...
    _actionMAIL(str, callback) {
        ...
        if (Number(str.charAt(0)) !== 2) {
            if (this._usingSmtpUtf8 && regexs.RESPONSE_550.test(str) && regexs.NON_ASCII_CHARS.test(this._envelope.from))
                message = '不允许国际化的邮箱名称';
            else message = '邮件命令失败';
    ...
    _actionRCPT(str, callback) {
        ...
        if (Number(str.charAt(0)) !== 2) {
            // 这是一个软错误
            if (this._usingSmtpUtf8 && regexs.RESPONSE_553.test(str) && regexs.NON_ASCII_CHARS.test(curRecipient)) message = '不允许国际化的邮箱名称';
