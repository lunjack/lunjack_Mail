...
class DataStream extends Transform {
    ...
    _transform(chunk, encoding, done) {
        ...
        for (let i = 0; i < chunk.length; i++) {
            if (chunk[i] === 0x2e) {
                ...
                if ((i && chunk[i - 1] === 0x0a) || (!i && (!this.lastByte || this.lastByte === 0x0a))) {
                    ...
                    chunks.push(buf), chunks.push(Buffer.from('.'));  // 在行首的点号前额外添加一个点号进行转义
                ...
                if ((i && chunk[i - 1] !== 0x0d) || (!i && this.lastByte !== 0x0d)) {
                    i <= lastPos ? chunklen += 2 : buf = chunk.slice(lastPos, i), chunks.push(buf), chunklen += buf.length + 2;
        ...
        this.lastByte = chunk.at(-1);  // 记录最后一个字节
        done();
    }
