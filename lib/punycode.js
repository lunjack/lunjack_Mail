...
function decode(input) {
    ...
    let i = 0, n = initialN, bias = initialBias; // 当前码点值,初始码点,初始偏差
    ...
    while (index < inputLen) {
        ...
        output.splice(i, 0, n), i++;                                     // 在指定位置插入解码后的字符,并移动到下一个插入位置
    }
...
function toUnicode(input) {
    return mapDomain(input, (string) => regexs.PUNYCODE.test(string) ? decode(string.slice(4).toLowerCase()) : string);
}
