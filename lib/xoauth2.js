...
class XOAuth2 extends Stream {
    constructor(options = {}, logger) {
        ...
        this.renewing = false, this.renewalQueue = []; // 跟踪是否正在续订及续订期间待处理请求的队列
        this.logger = getLogger({ logger }, { component }), this.accessToken = accessToken;
    ...
    getToken(renew, callback) {
        const { user, refreshToken, serviceClient } = this.options, { accessToken, xpires, provisionCallback, renewing } = this,
            // 重用令牌
            reuseToken = message => {
        ...
        const generateCallback = (err, accessToken) => {
            this.renewalQueue.forEach(item => item.callback(err, accessToken)), this.renewalQueue = [], this.renewing = false;
    ...
    updateToken(accessToken = '', timeout) {
        this.accessToken = accessToken, timeout = Math.max(Number(timeout) || 0, 0);
    ...
    generateToken(callback) {
        ...
        if (serviceClient) {
            ...
            const iat = Math.floor(Date.now() / 1000), // unix 时间
                tokenData = { iss: serviceClient, scope, sub: user, aud: accessUrl, iat, exp: iat + serviceRequestTimeout };
            let assertion;
            try {
            ...
            urlOptions = { grant_type, assertion }, loggedUrlOptions = { grant_type, assertion: tokenData };
        }
        else {
