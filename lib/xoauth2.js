 ...
 *     * accessToken 是一个字符串
 *     * expires 是可选的过期时间（毫秒）
 * 如果使用了 provisionCallback，则 Nodemailer 不会尝试自行生成令牌
 *
 * @constructor
    ...
    constructor(options = {}, logger) {
        ...
        if (options && options.serviceClient) {
            if (!options.privateKey || !options.user) {
                setImmediate(() => this.emit('error', new Error('服务账户需要 "privateKey" 和 "user" 选项！')));
                return;
            }
