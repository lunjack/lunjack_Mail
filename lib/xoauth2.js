...
const { getLogger, _handleReadableStream, nmfetch } = require('./shared');
...
class XOAuth2 extends Stream {
    constructor(options = {}, logger) {
        ...
        const { privateKey, user, serviceRequestTimeout, component, provisionCallback, accessToken, expires, timeout
            , accessUrl, customHeaders, customParams } = this.options;
        // 服务账户验证
    ...
    getToken(renew, callback) {
        ...
        if (provisionCallback)
            provisionCallback(user, !!renew, (err, accessToken, expires) => {
                if (!err && accessToken) {
    ...
    updateToken(accessToken, timeout) {
        ...
        this.emit('token', { user: this.options.user, accessToken: accessToken || '', expires: this.expires });
    }
    ...
    generateToken(callback) {
        ...
        const { serviceClient, scope, user, serviceRequestTimeout, clientId,
        ...
        if (serviceClient) {
            ...
            const tokenData = {
                iss: serviceClient, scope: scope || 'https://mail.google.com/',
                sub: user, aud: accessUrl, iat, exp: iat + serviceRequestTimeout
            };
            let assertion;
            try {
            ...
            loggedUrlOptions = {
                client_id: clientId || '', client_secret: (clientSecret || '').substring(0, 6) + '...',
                refresh_token: (refreshToken || '').substring(0, 6) + '...', grant_type
            };
        }

        // 添加自定义参数
        Object.assign(urlOptions, customParams);
        Object.assign(loggedUrlOptions, customParams);
        const tnx = 'OAUTH2';
        const action = 'post';
    ...
    postRequest(url, payload, params, callback) {
        ...
        const onceCallback = (err, data) => {
            if (returned) return;
            returned = true;
            callback(err, data);
        };

        req.on('readable', () => _handleReadableStream(req, chunks, chunklen));

        req.once('error', err => { onceCallback(err) });
        req.once('end', () => { onceCallback(null, Buffer.concat(chunks, chunklen.value)) });
    }
