...
const { getLogger, callbackPromise, PK, util, NET, dns, regexs, resetRegex, newURL } = require('../shared');
...
class Mail extends EventEmitter {
    constructor(transporter, options = {}, defaults = {}) {
        ...
        if (typeof this.transporter.on === 'function') {
            ...
            this.transporter.on('idle', (...args) => this.emit('idle', ...args))   // 发送器进入空闲状态
                .on('clear', (...args) => this.emit('clear', ...args));            // 发送器空闲且所有连接已终止
        }
    ...
    _processPlugins(step = '', mail, callback) {
        ...
        const processPlugins = () => {
            ...
            if (pos >= curplugins.length) {
                if (block === 'default' && pluginCount) block = 'user', pos = 0, curplugins = userPlugins;
    ...
    setupProxy(proxyUrl) {
        ...
        this.getSocket = (options, callback) => {
            ...
            const protocol = (protocol_.endsWith(':') ? protocol_.slice(0, -1) : protocol_).toLowerCase();
            // 检查是否有自定义代理处理器
            ...
            switch (protocol) {
                ...
                case 'socks4a': {
                    ...
                    const connect = ipaddress => {
                        ...
                        const type = Number(protocol_.replace(NON_DIGIT, '')) || 5;
                        ...
                        if (auth) {
                            ...
                            const password = decodeURIComponent(auth.split(':').pop());
