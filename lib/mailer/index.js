...
const { getLogger, callbackPromise } = require('../shared');
...
class Mail extends EventEmitter {
    constructor(transporter, options = {}, defaults = {}) {
        ...
        this._defaultPlugins = {
            compile: [(...args) => this._convertDataImages(...args)], // 编译阶段默认处理数据URL图片
            stream: []                                                // 流处理阶段暂无默认插件
        };
        ...
        if (proxy && typeof proxy === 'string') this.setupProxy(proxy);
    }
    ...
    sendMail(data, callback = null) {
        ...
        // 设置传输器的getSocket方法
        if (typeof this.getSocket === 'function') this._setupTransporterSocket();
        ...
        this.logger.debug(
            { tnx: 'transport', name, version, action: 'send' },
            '使用 %s/%s 发送邮件',
            name,
            version
        );
     ...
     */
    getVersionString() {
        const { name, version, homepage } = packageData;
        return util.format('%s (%s; +%s; %s/%s)', name, version, homepage, this.transporter.name, this.transporter.version);
    }
    ...
    _processPlugins(step, mail, callback) {
        ...
        if (userPlugins.length + defaultPlugins.length === 0) return callback(); // 如果没有插件，则直接返回
    ...
    setupProxy(proxyUrl) {
        ...
        this.getSocket = (options, callback) => {
            let protocol = proxy.protocol.replace(regexs.PROTOCOL_COLON, '').toLowerCase();
            ...
            switch (protocol) {
                ...
                case 'socks4a': {
                    ...
                    let connect = ipaddress => {
                        let proxyV2 = !!this.meta.get(moduleName).SocksClient;
                        let socksClient = proxyV2 ? this.meta.get(moduleName).SocksClient : this.meta.get(moduleName);
                        resetRegex(regexs.NON_DIGIT);
                        let proxyType = Number(proxy.protocol.replace(regexs.NON_DIGIT, '')) || 5;
                        let connectionOpts = {
                            proxy: {
                                ipaddress,
                        ...
                        if (proxy.auth) {
                            ...
                            else if (proxyType === 4) connectionOpts.userid = username; // 如果使用Socks4代理，则使用userid字段
                            else
                    ...
                    // 如果代理主机是IP地址，则直接连接
                    if (net.isIP(proxy.hostname)) return connect(proxy.hostname);
    ...
    _convertDataImages(mail, callback) {
        ...
        mail.resolveContent(mail.data, 'html', (err, html) => {
            ...
            resetRegex(regexs.DATA_URL_IMAGE);
            // 替换数据URL为CID引用
            html = (html || '')
                .toString()
                .replace(regexs.DATA_URL_IMAGE, (match, prefix, dataUri, mimeType) => {
                    let cid = randomBytes(10).toString('hex') + '@localhost';
                    // 添加附件
