...
class MailMessage {
    ...
    // 解析内容方法（代理给 shared.resolveContent）
    resolveContent(...args) {
    ...
    // 格式化列表URL
    _formatListUrl(url) {
        url = url.replace(/[\s<]+|[\s>]+/g, ''); // 去除空白和尖括号
        if (/^(https?|mailto|ftp):/.test(url)) return '<' + url + '>'; // 如果包含协议，则使用尖括号包裹

        if (/^[^@]+@[^@]+$/.test(url)) return '<mailto:' + url + '>'; // 如果包含@，则使用mailto协议
