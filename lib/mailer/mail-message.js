    ...
    resolveAll(callback) {
        ...
        if (alternatives && alternatives.length)
            alternatives.forEach((alternative, i) => {
                keys.push([alternatives, i]);
            });

        // 处理附件
        if (attachments && attachments.length) {
            attachments.forEach((attachment, i) => {
                let { filename, path, href, contentType } = attachment;;
                // 如果没有文件名，则从路径或href中提取，或使用默认名称
                if (!filename) {
                    filename = (path || href || '').split('/').pop().split('?').shift() || 'attachment-' + (i + 1);

                    // 如果没有扩展名，则根据内容类型添加
                    if (filename.indexOf('.') < 0) filename += '.' + detectExtension(contentType);
                    attachment.filename = filename; // 设置回attachment对象
                }

                // 如果没有内容类型，则根据文件名、路径或href检测
                if (!contentType) attachment.contentType = detectMimeType(filename || path || href || 'bin');
        ...
        let resolveNext = () => {
            ...
            let args = keys[pos++];
            // 父对象, 属性名, 当前项
            let parent = args[0], key = args[1], currentItem = parent[key];
    ...
    normalize(callback) {
        ...
        this.resolveAll((err, data) => {
            ...
            if (list && typeof list === 'object') {
                let listHeaders = this._getListHeaders(list);
