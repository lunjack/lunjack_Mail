...
const { resolveContent, regexs, resetRegex } = require('../shared');
...
class MailMessage {
    ...
    _buildMailData(data, defaults, options) {
        ...
        const mailData = { ...defaults, ...data, headers };
    ...
    resolveAll(callback) {
        ...
        // 如果有替代内容，则添加到键中
        const { alternatives, attachments } = this.data;
        if (alternatives && alternatives.length) alternatives.forEach((_, i) => keys.push([alternatives, i]));
        ...
        if (attachments && attachments.length) {
            attachments.forEach((attachment, i) => {
                ...
                if (!newFname) {
                    ...
                    if (newFname.indexOf('.') < 0) newFname += '.' + detectExtension(contentType); // 如果没有扩展名，则根据内容类型添加
        ...
        const mimeNode = new MimeNode();                                      // 创建 MimeNode 实例用于地址解析
        const addressKeys = ['from', 'to', 'cc', 'bcc', 'sender', 'replyTo']; // 处理地址字段
        ...
        addressKeys.forEach(address => {
            ...
            else if (address in this.data) this.data[address] = null; // 否则如果数据中有，则删除数据中的地址字段
        });
        ...
        const resolveNext = () => {
            ...
            const args = keys[pos++];
            // 父对象, 属性名, 当前项
            const parent = args[0], key = args[1], currentItem = parent[key];
    ...
    // 规范化邮件数据
    normalize(callback) {
        const envelope = this.data.envelope || this.message.getEnvelope(); // 获取信封信息
        const messageId = this.message.messageId();                        // 获取消息ID
        ...
        this.resolveAll((err, data) => {
            ...
            const convertToBase64 = (item) => {
                ...
                if (Buffer.isBuffer(content)) {
                    item.content = content.toString('base64');
            ...
            if (icalEvent) convertToBase64(icalEvent);                        // 处理日历事件
            if (alternatives?.length) alternatives.forEach(convertToBase64);  // 处理替代内容
            if (attachments?.length) attachments.forEach(convertToBase64);    // 处理附件
    ...
    setPriorityHeaders() {
        ...
        if (config) {
            this.message.setHeader('X-Priority', config[0]);
            this.message.setHeader('X-MSMail-Priority', config[1]);
    ...
    setListHeaders() {
        ...
        // 如果没有消息或没有列表头部，则直接返回
        if (!this.message || !list || typeof list !== 'object') return;
    ...
    _getListHeaders(listData) {
        const processedEntries = Object.entries(listData).map(([key, values]) => [key.toLowerCase().trim(), values]);
    ...
    // 格式化列表URL
    _formatListUrl(url) {
        const CLEAN = resetRegex(regexs.URL_CLEAN);
        url = url.replace(CLEAN, '');                               // 去除空白和尖括号
