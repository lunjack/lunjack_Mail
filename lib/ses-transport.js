...
const { getLogger, callbackPromise, formatRecipients, handleReadableStream, PK } = require('./shared');
...
class SesTransport extends EventEmitter {
    ...
    send(mail, callback) {
        ...
        const envelope = data.envelope || message.getEnvelope();
        const messageId = message.messageId(), { from, to } = envelope;
        ...
        try {
            const fromHeader = message._headers?.find(header => header.key.toLowerCase() === 'from');
        ...
        const getRawMessage = next => {
            ...
            const d = data._dkim || {}, dMsgId = 'date:message-id';
            d.skipFields = (d.skipFields && typeof d.skipFields === 'string') ? `${d.skipFields}:${dMsgId}` : dMsgId;

            // 创建消息流
            const sourceStream = message.createReadStream();
            ...
            stream.on('readable', () => handleReadableStream(stream, chunks, chunklen))
                .once('error', err => next(err)).once('end', () => next(null, Buffer.concat(chunks, chunklen.value)));
        };
