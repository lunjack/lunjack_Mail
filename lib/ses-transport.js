...
const { getLogger, callbackPromise, formatRecipients, PK } = require('./shared');
...
class SesTransport extends EventEmitter {
    ...
    send(mail, callback) {
        ...
        try {
            const fromHeader = mail.message._headers?.find(header => /^from$/i.test(header.key));
        ...
        let getRawMessage = next => {
            ...
            const d = mail.data._dkim = mail.data._dkim || {};
            d.skipFields = (d.skipFields && typeof d.skipFields === 'string') ? `${d.skipFields}:date:message-id` : 'date:message-id';
