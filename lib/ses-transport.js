...
class SesTransport extends EventEmitter {
    ...
    send(mail, callback) {
        ...
        const { messageId, _dkim = {}, from, to, readStream, ses = {} }
        ...
        const getRawMessage = next => {
            ...
            const sF = _dkim.skipFields, dMsgId = 'date:message-id';
            _dkim.skipFields = (sF && typeof sF === 'string') ? `${sF}:${dMsgId}` : dMsgId;

            const stream = readStream.pipe(new LeWindows()); // 创建消息流
            resolveStream(stream, next);                     // 将流数据读取到Buffer中
        };
