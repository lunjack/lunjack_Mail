...
class MailComposer {
    ...
    getAlternatives() {
        ...
        if (this.mail.html) {
            if (typeof this.mail.html === 'object' && (this.mail.html.content || this.mail.html.path || this.mail.html.href || this.mail.html.raw))
                html = this.mail.html; // 如果是对象并且至少有一个属性,则假定它是一个附件对象
            // 否则将其视为文本内容
        ...
        []
            ...
            .forEach(alternative => {
                ...
                if (alternative.filename) data.filename = alternative.filename; // 如果存在，则使用用户定义的文件名
                ...
                if (alternative.raw) data.raw = alternative.raw; // 如果存在，则使用原始内容
                // 如果存在，则使用路径
                ...
                else data.content = alternative.content || ''; // 否则使用内容

                if (alternative.encoding) data.encoding = alternative.encoding; // 如果存在，则使用编码

                if (alternative.headers) data.headers = alternative.headers; // 如果存在，则使用头部
    ...
    _createMixed(parentNode) {
        ...
        if (this._useAlternative) this._createAlternative(node); // 如果存在，则创建替代节点
        else if (this._useRelated) this._createRelated(node); // 否则，如果存在，则创建相关节点
    ...
    _createAlternative(parentNode) {
        ...
        this._alternatives.forEach(alternative => {
            ...
            else this._createContentNode(node, alternative); // 否则，创建内容节点
        });
    ...
    _createContentNode(parentNode, element) {
        ...
        // 如果内容是字符串，则将其转换为Buffer
        if (typeof element.content === 'string' && !['utf8', 'usascii', 'ascii'].includes(encoding))
            element.content = Buffer.from(element.content, encoding);

        // 优先使用预生成的原始内容(如果有)
        if (element.raw) node.setRaw(element.raw);
        else node.setContent(element.content); // 否则，使用内容
    ...
    _processDataUrl(element) {
        ...
        if (!dataUrl.startsWith('data:')) return element; // 如果不是数据URL，则返回原始元素

        // 如果数据URL大于100KB，则返回空内容
        ...
        if (!parsedDataUri) return element; // 如果没有解析数据URI，则返回原始元素

        element.content = parsedDataUri.data;
        element.contentType = element.contentType || parsedDataUri.contentType;

        if ('path' in element) element.path = false; // 如果有path，则将其设置为false

        if ('href' in element) element.href = false; // 如果有href，则将其设置为false
