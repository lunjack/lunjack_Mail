...
/** 正则表达式 */
const regexPunycode = /^xn--/;                                // 匹配Punycode前缀
const regexNonASCII = /[^\0-\x7F]/;                          // 匹配非ASCII字符（注意：U+007F DEL也被排除）
const regexSeparators = /[\x2E\u3002\uFF0E\uFF61]/g;         // RFC 3490 分隔符
...
const stringFromCharCode = String.fromCharCode; // 字符代码转字符串函数
...
const decode = function (input) {
    ...
    for (let index = basic > 0 ? basic + 1 : 0; index < inputLength /* 无最终表达式 */;) {
...
const punycode = {
    /**
     * 表示当前Punycode.js版本号的字符串。
     * @memberOf punycode
     * @type String
     */
    version: '2.3.1',
    /**
     * 用于从JavaScript内部字符表示（UCS-2）转换为Unicode码点，以及反向转换的方法对象。
     * @see <https://mathiasbynens.be/notes/javascript-encoding>
     * @memberOf punycode
     * @type Object
     */
