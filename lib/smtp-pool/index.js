...
class SmtpPool extends EventEmitter {
    constructor(options = {}) {
        ...
        // 速率限制配置
        this._rateLimit = {
            counter: 0, timeout: null, waiting: [], checkpoint: false,      // 计数器,超时定时器,等待队列,检查点
            delta: Number(rateDelta) || 1000, limit: Number(rateLimit) || 0 // 时间窗口（毫秒）,限制数量
        };
        this._queue = [], this._connections = []; // 消息队列,连接池
        this._connectionCounter = 0;              // 连接计数器
        this._closed = false, this.idling = true; // 连接池是否已关闭,是否处于空闲状态
    ...
    _processMessages() {
        ...
        const rCMI = resetRegex(regexs.CLEAN_MESSAGE_ID);
        const messageId = entry.messageId = (entry.mail.message.getHeader('message-id') || '').replace(rCMI, '');

        connection.available = false; // 标记连接为忙碌
        const cid = connection.id;
        const _rateLimit = this._rateLimit;
    ...
    _createConnection() {
        ...
        connection.on('available', () => {
            ...
            // 如果已关闭，则从连接列表中移除此连接,否则检查是否还有其它要发送的内容
    ...
    _requeueOnClose(connection) {
        ...
        this.logger.debug(
            { tnx: 'pool', cid: id, messageId, action: 'requeue' },
    ...
    _checkRateLimit(callback) {
        const now = Date.now();
        const { limit, counter, waiting, checkpoint, delta, timeout } = this._rateLimit;
