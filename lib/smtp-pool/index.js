...
const { regexs, resetRegex, initSmtpConstructor, verifySmtp } = require('../shared');
...
class SmtpPool extends EventEmitter {
    constructor(options = {}) {
        ...
        const { maxConnections, maxMessages, rateLimit, rateDelta } = this.options;
        this.options.maxConnections = maxConnections || 5; // 最大连接数
        this.options.maxMessages = maxMessages || 100;     // 每个连接最大消息数


        // 速率限制配置(计数器,超时定时器,等待队列,检查点,时间窗口->毫秒,限制数量)
        this._rateLimit = {
            counter: 0, timeout: null, waiting: [], checkpoint: false,
            delta: Number(rateDelta) || 1000, limit: Number(rateLimit) || 0
        };
        // 消息队列,连接池,连接计数器,连接池是否已关闭,是否处于空闲状态
        this._queue = [], this._connections = []; this._connectionCounter = 0; this._closed = false, this.idling = true;
    ...
    send(mail, callback) {
        ...
        this._queue.push({ mail, requeueAttempts: 0, callback });  // 将邮件加入队列(包括重试尝试计数)
     ...
     */
    close() {
        this._closed = true, clearTimeout(this._rateLimit.timeout); // 标记为已关闭并清除速率限制定时器（如果存在）
    ...
    _processMessages() {
        let connection;
        const queLen = this._queue.length, conLen = this._connections.length,
            idlingEmit = () => {
                this.idling = true, this.emit('idle'); // 标记为空闲
            };
        ...
        if (!connection && conLen < maxConnections) connection = this._createConnection();
        if (!connection) return this.idling = false;               // 如果没有可用连接，则标记为非空闲并返回
        if (!this.idling && queLen < maxConnections) idlingEmit(); // 检查处理队列中是否有空闲空间

        // 从队列中取出一个条目并分配给连接
        const entry = connection.queueEntry = this._queue.shift(), rCMI = resetRegex(regexs.CLEAN_MESSAGE_ID),
            messageId = entry.messageId = (entry.mail.message.getHeader('message-id') || '').replace(rCMI, ''),
            cid = connection.id, _rateLimit = this._rateLimit;
        connection.available = false; // 标记连接为忙碌
        this.logger.debug({ tnx: 'pool', cid, messageId, action: 'assign' },
            '将消息 <%s> 分配给 #%s (%s)', messageId, cid, connection.messages + 1
        );
        // 处理速率限制
        if (_rateLimit.limit) {
            _rateLimit.counter++;
            if (!_rateLimit.checkpoint) _rateLimit.checkpoint = Date.now();
        }
     ...
     */
    _createConnection() {
        const connection = new PoolResource(this), cid = connection.id = ++this._connectionCounter; // 分配连接ID
        ...
        connection.on('available', () => {
            ...
            this._closed ? this.close() : this._processMessages(); // 如果已关闭,则从连接列表中移除此连接,否则检查是否还有其它发送
        }).once('error', err => {
            ...
            this._removeConnection(connection), this._continueProcessing(); // 从连接列表中移除错误的连接,然后继续处理队列
        }).once('close', () => {
    ...
    _requeueOnClose(connection) {
        ...
        const { messageId, requeueAttempts } = queueEntry;
        this.logger.debug({ tnx: 'pool', cid: id, messageId, action: 'requeue' },
            '为 #%s 重新排队消息 <%s>。尝试次数: #%s', messageId, id, requeueAttempts);
        this._queue.unshift(queueEntry), connection.queueEntry = false;
    }
    ...
    _checkRateLimit(callback) {
        const now = Date.now(), { limit, counter, waiting, checkpoint, delta, timeout } = this._rateLimit;
        if (!limit) return callback();
        if (counter < limit) return callback();

        waiting.push(callback);
        if (checkpoint <= now - delta) return this._clearRateLimit();
        else if (!timeout) {
            this._rateLimit.timeout = setTimeout(() => this._clearRateLimit(), delta - (now - checkpoint));
    ...
    _clearRateLimit() {
        ...
        this._rateLimit.timeout = null, this._rateLimit.counter = 0, this._rateLimit.checkpoint = false;

        const { waiting } = this._rateLimit;
        while (waiting.length) setImmediate(waiting.shift()); // 恢复所有暂停的连接
    }
     ...
     */
    verify(callback) {
        const auth = new PoolResource(this).auth; // 使用PoolResource的认证信息
        return verifySmtp(this, callback, SmtpConnection, auth);
    }
}
