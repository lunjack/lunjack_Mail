...
class PoolResource extends EventEmitter {
            ...
            connection.connect(() => {
                ...
                if (this.auth && (connection.allowsAuth || newOp.forceAuth))
                    connection.login(this.auth, err => {
                        if (returned) return;
                        returned = true, this._isCallbackErr(err, callback);
    ...
    send(mail, callback) {
        ...
        const { data, message } = mail, { dsn } = data;
        const envelope = message.getEnvelope();                           // 获取信封信息
        ...
        // 执行邮件发送
        this.connection.send(envelope, message.createReadStream(), (err, info) => {
