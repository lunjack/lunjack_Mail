...
function assign(target, ...sources) {
    ...
    for (const source of sources) {
        ...
        for (const [key, value] of Object.entries(source)) {
            ...
            target[key] = ['tls', 'auth'].includes(key) && value && typeof value === 'object' ? { ...target[key], ...value } : value;
        }
    }
    return target;
}
 ...
 */
function createProxyConnection(options, logger, socketOptions) {
    ...
    const connection = new SmtpConnection(newOptions);  // 创建SMTP连接实例
 ...
 */
function initSmtpConstructor(instance, options, defaultComponent) {
    // 如果选项是字符串，则将其视为URL
    ...
    if (typeof options.getSocket === 'function') instance.getSocket = options.getSocket;
    ...
    // 创建临时连接对象用于获取版本信息
    let connection = new SmtpConnection(instance.options);
    instance.version = packageData.version + '[client:' + connection.version + ']';

    // 创建日志记录器
    instance.logger = getLogger(instance.options, {
        component: instance.options.component || defaultComponent
    });
}
