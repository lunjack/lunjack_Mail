...
function _createDefaultLogger(levels) {
    ...
    let print = (level, entry, message, ...args) => {
        ...
        // 分行打印（处理多行消息）
        message.split(regexs.HEADER_LINE_BREAK).forEach(line => {
            console.log('[%s] %s %s',
...
function formatRecipients(recipients) {
    const rList = [].concat(recipients || []);
    // 仅显示前3个收件人，超过部分用省略号表示
    return rList.length > 3 ? rList.push('...以及另外 ' + rList.splice(2).length + ' 个收件人') : rList;
}
