...
function _resolveStream(stream, callback) {
    let responded = false, chunks = [], chunklen = 0;
    function cleanup() {
        if (responded) return;
        responded = true;
    };
...
function getLogger(options = {}, defaults) {
    ...
    // 如果没有提供日志记录器，创建空实现
    if (!options.logger) return Object.fromEntries(levels.map(level => [level, () => false]));

    // 如果logger为true，创建控制台日志记录器
    const logger = options.logger === true ? _createDefaultLogger(levels) : options.logger;

    // 包装日志方法
    return Object.fromEntries(
        levels.map(level => [level, (data, message, ...args) => _logFunc(logger, level, defaults, data, message, ...args)])
    );
}
...
function resolveHostname(options = {}, callback) {
    const { host, dnsTtl } = options;
    // 如果没有host但有servername，使用servername作为host
    const newHost = options.host = !host && options.servername ? options.servername : host;
    const servername = options.servername || newHost;

    // 如果host是IP地址或为空，直接返回
    if (!newHost || net.isIP(newHost)) {
        const value = { addresses: [newHost], servername: servername || false };
        return callback(null, _formatDNSValue(value, { cached: false }));
    }

    // 检查DNS缓存
    let cached;
    if (dnsCache.has(newHost)) {
        cached = dnsCache.get(newHost);
        // 如果缓存未过期，使用缓存值
    ...
    function handleSuccess(addresses) {
        ...
        dnsCache.set(newHost, { value, expires: Date.now() + (dnsTtl || DNS_TTL) });  // 更新DNS缓存
        return callback(null, _formatDNSValue(value, { cached: false }));
    }
...
function parseDataURI(uri) {
    ...
    const metaStr = uri.slice(5, commaIndex);     // 'data:'.length = 5
...
function resolveContent(data, key, callback) {
    ...
    if (typeof content === 'object') {
        ...
        else if (regexs.HTTP_URL.test(content.path || content.href)) {
            contentStream = nmfetch(content.path || content.href);
            return _resolveStream(contentStream, callback);
        }
        // 否则,如果content是Data URI，则解析Data URI
        else if (regexs.DATA_URL.test(content.path || content.href)) {
...
function assign(target, ...sources) {
    ...
    sources.forEach(source => {
        ...
        for (const [key, value] of Object.entries(source)) {
            ...
            if (['tls', 'auth'].includes(key) && value && typeof value === 'object') target[key] = { ...target[key], ...value };
...
function encodeXText(str) {
    if (!regexs.NEED_ENCODING.test(str)) return str; // 如果字符串中不存在需要编码的字符，直接返回原字符串
    ...
    for (let i = 0, len = buf.length; i < len; i++) {
        let c = buf[i];
        // 检查当前字节是否需要编码(控制字符、非ASCII字符、加号、等号)
        if (c < 0x21 || c > 0x7E || c === 0x2B || c === 0x3D) result += '+' + c.toString(16).toUpperCase().padStart(2, '0');
        else result += String.fromCharCode(c); // 安全字符直接添加到结果中
    }
    return result;
}
 ...
 */
function createProxyConnection(options, logger, socketOptions, ConnectionClass) {
    ...
    if (socketOptions?.connection) {
        ...
        newOptions = assign(false, options);
        Object.assign(newOptions, socketOptions);
    }

    const connection = new ConnectionClass(newOptions);  // 创建SMTP连接实例
