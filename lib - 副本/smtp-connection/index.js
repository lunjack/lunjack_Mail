...
const { getLogger, resolveHostname, callbackPromise, encodeXText, PK, OS, NET, regexs, resetRegex } = require('../shared');
...
class SmtpConnection extends EventEmitter {
    constructor(options = {}) {
        ...
        this.id = randomBytes(8).toString('base64').replace(rUSB, '');
        this.stage = 'init';
        this.options = options;
        this.secureConnection = !!this.options.secure, this.alreadySecured = !!this.options.secured;
        ...
        // 设置日志记录器
        this.logger = getLogger(this.options, { component: this.options.component || 'smtpConnection', sid: this.id });
    ...
    connect(connectCallback) {
        if (typeof connectCallback === 'function') {
            this.once('connect', () => {
                this.logger.debug({ tnx: 'smtp' }, 'SMTP握手完成');
                connectCallback();
            });
    ...
    close() {
        ...
        const socket = this._socket?.socket ?? this._socket;
    ...
    */
    _generateAuthPlainString(includePassword = true) {
        const { pass, user } = this._auth.credentials;
        const credentials = includePassword ? pass : '/* secret */';
    ...
    login(authData = {}, callback) {
        ...
        if (this.customAuth.has(this._authMethod)) {
            ...
            let lastResponse, returned = false;
            function cleanup() {
                if (returned) return;
                returned = true;
            };
    ...
    _onData(chunk) {
        ...
        const _rQueue = this._responseQueue;
        let lastline = _rQueue.at(-1) ?? null;
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i];
            // 检查是否需要延续（确保 lastline 存在）
            if (lastline && regexs.CONTINUATION_RESPONSE.test(lastline.split('\n').pop()))
                lastline += `\n${line}`, _rQueue[_rQueue.length - 1] = lastline;
            else _rQueue.push(line), lastline = line; // 新响应：推入队列并更新 lastline
        }
    ...
    _formatError(message, type, response, command) {
        ...
        const responseCode = (typeof response === 'string' && parseInt(response)) || false;
    ...
    _onClose() {
        ...
        const _remainder = this._remainder;
        const { debug, transactionLog } = this.options;
        if (_remainder?.trim()) {
    ...
    _upgradeConnection(callback) {
        ...
        try {
            const tlsSocket = TLS.connect(opts, () => {
                this.secure = true, this.upgrading = false;
            ...
            tlsSocket.on('error', this._onSocketError).once('close', this._onSocketClose).once('end', this._onSocketEnd);
    ...
    _setDsnEnvelope(params) {
        ...
        if (notify) {
            ...
            const invaliNotify = notify.filter(n => !validNotify.includes(n));
    ...
    _createSendStream(callback) {
        ...
        const _rActions = this._responseActions;
        if (this.options.lmtp)
            accepted.forEach((recipient, i) => {
                _rActions.push(str => this._actionLMTPStream(recipient, i === lastIndex, str, callback));
            });
        else _rActions.push(str => this._actionSMTPStream(str, callback));
    ...
    _actionEHLO(str) {
        ...
        const { requireTLS, ignoreTLS } = this.options;
        if (!str.startsWith('2')) {
            if (requireTLS)
        ...
        const authMechanismSource = regexs.AUTH_MECHANISM.source;
        // 检测服务器是否支持PLAIN认证
        ...
        const match = str.match(regexs.SIZE)
    ...
    _actionAUTH_CRAM_MD5(str, callback) {
        ...
        const hmacMD5 = createHmac('md5', pass);
        ...
        this._sendCommand(
            ...
            Buffer.from(`${user} /* secret */`).toString('base64')  // 在日志中的密码隐藏哈希
        );
    }
    ...
    _actionAUTH_PLAIN_RESPONSE(str, isRetry, callback) {
        ...
        this.logger.info({ tnx: 'smtp', username: user, action: 'authenticated', method },
            '用户 %s 已认证', JSON.stringify(user)
        );
        this.authenticated = true, callback(null, true);
    }
    ...
    _actionRCPT(str, callback) {
        const { _envelope, _recipientQueue, _responseActions } = this;
        const { rejected, rejectedErrors, accepted, rcptQueue, to } = _envelope;
        let err, curRecipient = _recipientQueue.shift();
        if (!str.startsWith('2')) {
            const message = this._usingSmtpUtf8 && str.startsWith('553 ') && regexs.NON_ASCII.test(curRecipient) ?
                '不允许国际化的邮箱名称' : '收件人命令失败';
        ...
        if (!rcptQueue.length && !_recipientQueue.length) {
            if (rejected.length < to.length) {
                _responseActions.push(str => this._actionDATA(str, callback));
            ...
            curRecipient = rcptQueue.shift();
            _recipientQueue.push(curRecipient);
