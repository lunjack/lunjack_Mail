...
class RelaxedBody extends Transform {
    ...
    updateHash(chunk) {
        ...
        if (this.remainder) {
            const rMW = resetRegex(regexs.MULTIPLE_WSP); // 余数中的空白字符处理（但不移除行尾空白，因为可能不是完整行）
            const pRemainder = this.remainder.replace(rMW, ' ');
    ...
    _flush(callback) {
        try {
            ...
            if (this.remainder) {
                ...
                const pRemainder = this.remainder.replace(regexs.TRAILING_WSP, '').replace(rMW, ' ');
