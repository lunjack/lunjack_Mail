...
class SesTransport extends EventEmitter {
    ...
    send(mail, callback) {
        ...
        const getRawMessage = next => {
            ...
            stream.once('error', err => next(err)).once('end', () => next(null, Buffer.concat(chunks, chunklen.value)));
        };
            ...
            getRawMessage((err, raw) => {
                ...
                this.getRegion((err, region) => {
                    ...
                    this.ses.sesClient.send(command).then(data => {
                        ...
                        callback(null, {
                            envelope: { from, to },
                            messageId: `<${data.MessageId}${(!data.MessageId.includes('@') ? `@${region}.amazonses.com` : '')}>`,
